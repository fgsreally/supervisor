...
export interface ExternalSessionCandidate {
  ...
  preview: string;
  lastActiveAt: string;
  /** Already imported into a wecode session. */
  imported?: boolean;
  ...
  for (const entry of entries) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (typeof part.importPath === "string" && part.importPath) {
        saved = await copyPathIntoSessionMedia(projectId, sessionId, part.importPath, {
          name: typeof part.name === "string" ? part.name : undefined,
        });
      }
      if (!saved && typeof part.importDataUrl === "string" && part.importDataUrl) {
        saved = await writeDataUrlIntoSessionMedia(projectId, sessionId, part.importDataUrl, {
          name: typeof part.name === "string" ? part.name : undefined,
        });
      }
      if (!saved) {
