...
import { getWecodeSettings, type WecodeSettings } from "@/api";
...
let cachedSettings: { at: number; value: WecodeSettings } | null = null;

async function loadSpeechSettings(): Promise<WecodeSettings> {
  const now = Date.now();
  if (cachedSettings && now - cachedSettings.at < SETTINGS_CACHE_MS) {
    return cachedSettings.value;
  }
  const value = await getWecodeSettings();
...
export function useVoiceRecognition(
  ...
  function connect(language: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const webPassword = localStorage.getItem("wecode-web-password");
