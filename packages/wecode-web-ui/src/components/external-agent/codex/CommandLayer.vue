  ...
  <Teleport to="body">
    <div v-if="open" class="codex-model-overlay" @click.self="close">
      ...
      >
        <header>
          ...
          <button type="button" :title="t('common.close')" :aria-label="t('common.close')" @click="close"><X class="h-5 w-5" /></button>
        </header>
    ...
    <div v-if="commandOpen" class="codex-model-overlay" @click.self="closeCommand">
      ...
      >
        <header>
          <h2>{{ commandTitle }}</h2>
          <button type="button" :title="t('common.close')" :aria-label="t('common.close')" @click="closeCommand"><X class="h-5 w-5" /></button>
        </header>
        ...
        <template v-else>
          ...
          <CodexStatusView v-if="isStatus" :data="commandData ?? {}" />
          <HighlightedCodeBlock v-else-if="commandResult" :code="commandResult" lang="json" class="command-result" />
...
import CodexStatusView from "./CodexStatusView.vue";
...
const isStatus = computed(() => commandName.value === "status" && Boolean(commandData.value?.thread));
