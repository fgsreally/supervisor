...
type RouteHandler = (context: WecodeHttpContext) => unknown | Promise<unknown>;
...
export interface WecodeHttpContext {
  req: {
    raw: Request;
...
export type WecodeElysiaApp = Elysia & {
  request(path: string, init?: RequestInit): Promise<Response>;
};

function createContext(context: ElysiaRequestContext): WecodeHttpContext {
  ...
  } as WecodeHttpContext;
}

export class WecodeElysiaBuilder {
  readonly app = new Elysia({ adapter: node() }).use(
    openapi({
      path: "/openapi",
      documentation: {
        info: { title: "WeCode HTTP API", version: "1.0.0" },
      },
    }),
  );

  constructor(
    private readonly requestGuard?: (context: WecodeHttpContext) => Response | undefined,
  ) {}
  ...
  private route(
    ...
    const adapted = (context: ElysiaRequestContext) => {
      const wecodeContext = createContext(context);
      return this.requestGuard?.(wecodeContext) ?? handler(wecodeContext);
    };
  ...
  build(): WecodeElysiaApp {
    const app = this.app as WecodeElysiaApp;
