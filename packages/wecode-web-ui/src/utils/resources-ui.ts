...
export function parseResourceId(id: string): { kind: UIResourceKind; path: string } | null {
  ...
  if (kind !== "skills" && kind !== "plugins" && kind !== "prompts" && kind !== "mcp") {
    return null;
  }
...
function pluginFilesToUi(
  pluginPath: string,
  files: Array<{ relativePath: string; content: string }>,
): UISkillFile[] {
  return files.map((file) => ({
    id: `${pluginPath}/${file.relativePath}`,
    fileName: file.relativePath.split("/").pop() ?? file.relativePath,
    content: file.content,
  }));
}
...
function layerToUiItems(
  ...
  for (const ext of layer.plugins) {
    const item: UIFileItem = {
      id: resourceId("plugins", ext.entryPath),
      kind: "plugins",
      layer: resourceLayer,
      ...
      files: pluginFilesToUi(ext.rootDir, ext.files),
...
export function agentResourcesToUiItems(
  ...
  const empty: ResourceLayer = { skills: [], prompts: [], plugins: [], mcp: [] };
  return [
