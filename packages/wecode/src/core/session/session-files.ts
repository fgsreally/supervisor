...
export function getWecodeProjectsRoot(): string {
  return join(getWecodeHome(), "projects");
}

export function getProjectDir(projectId: string | number): string {
  return join(getWecodeProjectsRoot(), String(projectId));
}
...
export function listProjectDirs(): string[] {
  const root = getWecodeProjectsRoot();
...
/** @deprecated use getWecodeProjectsRoot. */
export function getWecodeSessionsRoot(): string {
  return getWecodeProjectsRoot();
}
