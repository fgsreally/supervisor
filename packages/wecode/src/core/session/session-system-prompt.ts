import { appendContpluginFilesToSystemPrompt } from "../resource/context-files.js";
...
Session-owned files:
- Use @/path only with Wecode file tools such as read, ls, grep, and find. @/ is a logical path rooted at this Session's private directory.
...
Session-owned files:
- This external Agent does not have Wecode's logical file-tool path syntax. Session-owned paths use \${SV_SESSION_DIR}/path.
...
export function composeSessionSystemPrompt(options: {
  ...
  const withContext = appendContpluginFilesToSystemPrompt(parts.join("\n\n"), options.cwd);
