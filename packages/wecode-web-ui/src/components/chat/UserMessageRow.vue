  ...
  >
    <div class="max-w-[75%] flex flex-col items-end min-w-0">
      ...
      >
        ...
        <div v-if="false && pastedTexts.length" class="pasted-text-tags relative z-10">
          <button
        ...
        <div v-if="false && attachments.length" class="pasted-text-tags relative z-10">
          <span
        ...
        <div v-if="slashCommand" class="relative z-10 slash-message">
          ...
          <ChatRichText
            ...
            class="relative z-10"
            @image-click="openImageTag"
            @pasted-click="openPastedTextById"
          />
      </div>
    </div>
...
function openImageTag(label: string) {
  const match = /Image\s*#\s*(\d+)/i.exec(label);
  const index = match ? Number(match[1]) - 1 : 0;
  if (previewableImages.value.length) openPreview(Math.max(0, Math.min(index, previewableImages.value.length - 1)));
}
...
function openPastedTextById(id: string) {
  const item = pastedTexts.value.find((candidate) => candidate.id === id);
  if (item) openPastedText(item);
}
...
.user-message-images__thumb-btn:hover .user-message-images__label,
.user-message-images__thumb-btn:focus-visible .user-message-images__label {
