<template>
  <div
    ...
    @chat-image-remove="onImageRemove"
  >
...
const props = withDefaults(
  defineProps<{
    ...
    images?: Record<string, { previewUrl: string; name: string }>;
  }>(),
  {
    workspaceFiles: () => [],
    projects: () => [],
...
const emit = defineEmits<{
  ...
  "remove-image": [placeholder: string];
}>();
...
const imageCatalogCompartment = new Compartment();
...
function buildExtensions() {
  return [
    ...
    imageCatalogCompartment.of(imageCatalogFacet.of(props.images)),
    chatInputTagExtension,
...
watch(() => props.images, (images) => viewRef.value?.dispatch({ effects: imageCatalogCompartment.reconfigure(imageCatalogFacet.of(images)) }), { deep: true });
...
function onImageRemove(event: Event) {
  const placeholder = (event as CustomEvent<string>).detail;
  if (placeholder) emit("remove-image", placeholder);
}
