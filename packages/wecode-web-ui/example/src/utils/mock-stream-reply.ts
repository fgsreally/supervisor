...
function extractPathHint(userText: string): string {
  ...
  return match?.[0] ?? "packages/wecode-web-ui/example/src/views/ChatView.vue";
}
...
function buildTurnSteps(userText: string, agentName: string): StreamStep[] {
  ...
  return [
    ...
    },
    {
      ...
      name: "write",
      arguments: {
        path: "packages/wecode-web-ui/example/.stream-demo-last-request.md",
        content: `# Last stream demo request\n\n${userText}\n`,
      },
      resultText:
        "Wrote packages/wecode-web-ui/example/.stream-demo-last-request.md (48 bytes)",
      pauseMs: 380,
    },
    {
      kind: "text",
      text: "\n\n以上步骤按 SSE 节奏到达：`text` 增量 → `tool_call` → `tool_result` → `agent_end`。真实 wecode 通过 `/sessions/:id/events` 推送同类事件。",
    },
  ];
}
