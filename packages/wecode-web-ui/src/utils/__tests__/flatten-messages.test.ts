...
describe("buildDisplayGroups", () => {
  ...
  it("keeps the full turn timestamps when conclusion-only groups are merged", () => {
    const groups = applyConclusionOnlySplit([
      {
        id: "a1",
        type: "grouped_assistant",
        role: "assistant",
        pieces: [{ kind: "text", text: "start" }],
        createdAt: 1_000,
        endedAt: 1_500,
        assets: [],
      },
    ...
    expect(group?.type === "grouped_assistant" ? [group.createdAt, group.endedAt] : null).toEqual([
      1_000, 5_000,
    ]);
  });
});
