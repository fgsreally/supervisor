...
@available(iOS 16.1, *)
struct WecodeLiveActivityAttributes: ActivityAttributes {
...
final class LiveActivityManager {
    ...
    private var activities: [String: Activity<WecodeLiveActivityAttributes>] = [:]

    func start(from call: CAPPluginCall) {
        guard ActivityAuthorizationInfo().areActivitiesEnabled else { return }
        let sessionId = call.getString("sessionId") ?? UUID().uuidString
        let title = call.getString("title") ?? "Wecode"
        let subtitle = call.getString("subtitle") ?? ""
        let phase = call.getString("phase") ?? "thinking"
        let attributes = WecodeLiveActivityAttributes(sessionId: sessionId, title: title)
        let state = WecodeLiveActivityAttributes.ContentState(subtitle: subtitle, phase: phase)
        do {
