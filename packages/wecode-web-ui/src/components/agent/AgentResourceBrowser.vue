<template>
  <div class="resource-browser flex flex-1 min-h-0 overflow-hidden">
    ...
    <template v-else>
      <!-- Skills and Plugins with files: skill list + file tree -->
      <template v-if="kind === 'skills' || kind === 'plugins'">
        <div
          ...
          <div class="flex-1 min-h-0 min-w-0 overflow-y-auto overflow-x-hidden">
            ...
            >
              <div class="flex items-start gap-1 min-w-0">
                <ResourceListItem
                  :name="item.kind === 'skills' ? item.name : getFileBaseName(item.fileName)"
                  :path="item.kind === 'skills' ? item.rootPath : item.layer === 'global' ? item.path : undefined"
                />
          ...
          <div class="flex-1 min-h-0 min-w-0 overflow-y-auto overflow-x-hidden">
            ...
            >
              <ResourceListItem
                ...
                :path="item.layer === 'global' ? item.path : undefined"
              />
              <ResourceLayerBadge :layer="item.layer" />
            </div>
            <div
...
import ResourceContentView from "@/components/resource/ResourceContentView.vue";
import ResourceListItem from "@/components/resource/ResourceListItem.vue";
import ResourceLayerBadge from "@/components/resource/ResourceLayerBadge.vue";
import ResizeHandle from "@/components/base/ResizeHandle.vue";
import SkillFileTree from "@/components/resource/SkillFileTree.vue";
import TemplateSyntaxHelpButton from "@/components/resource/TemplateSyntaxHelpButton.vue";
...
const selectedSkill = computed(() => {
  ...
  if (props.kind === "plugins") {
    ...
    if (item && isFileItem(item) && item.files && item.files.length > 0) {
      // Treat plugin with files like a skill
...
const selectedFileItem = computed(() => {
  // For plugins with files, selectedFile handles it
  if (props.kind === "plugins") {
    const skill = selectedSkill.value;
