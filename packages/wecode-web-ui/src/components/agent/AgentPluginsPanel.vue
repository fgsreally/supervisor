<template>
  <div class="agent-ext">
    <div v-if="loading" class="agent-ext-loading">
      ...
      {{ t("agent.pluginsLoading") }}
    </div>
    <div v-else class="agent-ext-content custom-scrollbar">
      <div class="agent-ext-list">
        <div v-for="item in plugins" :key="item.resourceId" class="agent-ext-row">
          ...
          <div class="agent-ext-row__main">
            <div class="agent-ext-row__heading">
              ...
              <span class="agent-ext-badge">
                {{ item.builtin ? t("agent.builtin") : t("agent.addedPlugins") }}
              </span>
            </div>
        ...
        <div v-if="plugins.length === 0" class="agent-ext-empty">
          {{ t("agent.noUserPlugins") }}
        </div>
...
const plugins = ref<AgentPluginInfo[]>([]);
...
const builtinItems = computed(() => plugins.value.filter((item) => item.builtin));
const userItems = computed(() => plugins.value.filter((item) => !item.builtin));
...
async function reload() {
  loading.value = true;
  try {
    [catalog.value, plugins.value] = await Promise.all([
      listResourceCatalog("plugin"),
      agentStore.fetchAgentPlugins(props.agentId),
    ]);
  } finally {
    loading.value = false;
  }
...
async function remove(item: AgentPluginInfo) {
  if (item.builtin) return;
  removingId.value = item.resourceId;
  try {
    await agentStore.unbindAgentResource(props.agentId, item.resourceId);
    plugins.value = plugins.value.filter((row) => row.resourceId !== item.resourceId);
