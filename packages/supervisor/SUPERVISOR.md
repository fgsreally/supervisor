...
随包安装的内置 agent prompt。启动时由 `internal-agents.ts` 写入 agent home 的 `SYSTEM.md`。

| 文件 | 用途 |
|------|------|
| `shadow/prompt.md` | Shadow 协作者（`is_internal`）：每轮主对话结束后分析记忆与安全，按需向父 session 投递消息；**不可**用于创建用户 session |
| `intro/prompt.md` | Intro 引导助手（非 internal）：教学、答疑、查看资源；**可以**创建用户 session |
...
| 文件 | 用途 |
|------|------|
| `builtin-assistant-system.md` | 默认 assistant 系统提示主体 |
...
| `session-runtime.ts` | **遗留** runtime 包装（无扩展集成）；实际使用 `core/session-runtime.ts` |

---

### `src/agent/` — Agent 资源与配置

| 文件 | 用途 |
|------|------|
| `agent-paths.ts` | Agent home / 全局资源目录路径、`~/.pi/supervisor/agents/` 布局 |
| `agent-resources.ts` | 加载 skills、prompts、extensions；解析 agent 工具列表 |
| `builtin-assistant.ts` | 确保默认 builtin assistant agent 存在 |
| `built-in-providers.ts` | 内置 LLM provider 定义 |
| `context-files.ts` | 从工作区收集上下文文件（`.cursorrules` 等） |
| `extension-installer.ts` | 从 npm / 本地路径安装扩展到全局目录 |
| `extension-source.ts` | 解析扩展来源（全局、agent、项目） |
| `internal-agents.ts` | 注册随包 agent（shadow 内部、intro 用户可 spawn）；`agents.is_internal` 控制是否允许建 session |
| `prompt-templates.ts` | 加载与展开 `/prompt:name` 模板 |
| `skills.ts` | 扫描与格式化 agent skills |
| `system-prompts.ts` | 组装完整系统提示（读取 `prompts/` 目录） |
...
| 文件 | 用途 |
|------|------|
| `session-manager.ts` | **中心协调器**：spawn、prompt、输入队列、状态、扩展、MCP、成员 agent |
| `session-runtime.ts` | 单个 session 的 harness 包装：prompt/abort、扩展运行时、工具挂载 |
| `session-input-queue.ts` | 每 session 统一输入队列；`level >= 90` 打断当前轮次 |
| `session-storage.ts` | AgentHarness 与 SQLite 消息树的读写适配 |
| `session.ts` | 轻量 session 包装（部分旧路径仍在使用） |
...
| `session-log-file.ts` | Session 运行日志文件写入 |
| `review.ts` | Code review 相关辅助逻辑 |
...
|------|------|
| `config.ts` | 压缩阈值与策略配置 |
| `origin.ts` | 判断消息来源（用户 prompt vs 工具回调等） |
| `projector.ts` | 将 session 树投影为可压缩的消息序列 |
| `micro.ts` | 微压缩（单轮摘要） |
...
### `src/extension-system/` — 扩展框架

| 文件 | 用途 |
|------|------|
| `index.ts` | 扩展系统对外导出 |
| `define-extension.ts` | `defineExtension()` 辅助，声明扩展元数据与 setup |
| `loader.ts` | 扫描目录、动态加载扩展模块（jiti） |
| `runtime.ts` | **ExtensionRuntime**：事件总线、工具注册、生命周期 |
| `extension-deps.ts` | 注入给扩展的 `ctx`：`sendUserMessage`、`spawn`、`exec` 等 |
| `extension-entry.ts` | 扩展入口解析与校验 |
| `extension-event-bridge.ts` | AgentHarness 事件 → 扩展事件的映射 |
| `extension-session-services.ts` | 扩展可用的 session 级服务 |
| `register-agent-tool.ts` | 向 agent 注册扩展工具的辅助 |
| `tool-adapter.ts` | 将 `ToolInfo` 转为 `AgentTool` 并包装执行 |
| `tool-policy.ts` | 工具启用/禁用策略 |
| `tool-probe.ts` | 探测扩展工具 schema（用于 GetMcpTools 类场景） |
| `types.ts` | 扩展 API 类型：`ExtensionContext`、`ToolInfo`、`SpawnSessionRequest` 等 |

#### `src/extension-system/extensions/` — 内置扩展
...
用户安装的第三方扩展通常位于 `~/.pi/supervisor/extensions/` 或 agent home 下。
...
|------|------|
| `index.ts` | MCP 模块导出 |
| `mcp-config-loader.ts` | 从配置加载 MCP server 列表 |
| `mcp-client-manager.ts` | 管理 MCP 客户端连接生命周期 |
| `mcp-extension.ts` | 将 MCP server 暴露为 session 扩展 |
| `mcp-tool.ts` | MCP 工具调用适配 |
| `mcp-types.ts` | MCP 相关类型 |

---

### `src/supervisor-resources/` — `pi-supervisor://` 资源

供子 agent / 扩展读取 session 消息、agent skills 等。

| 文件 | 用途 |
|------|------|
| `constants.ts` | 资源 scheme 常量 |
| `urls.ts` | 构造资源 URL 的辅助函数 |
| `parse.ts` | 解析 `pi-supervisor://...` URL |
| `list.ts` | 列出某 session / agent 下可用资源 |
| `read.ts` | 读取资源内容（消息、skill、prompt 等） |
| `index.ts` | 模块 re-export |

`tools/supervisor-resources/` 为 agent 侧工具封装（路径常量等）。
...
| `loader.ts` | 扫描 agent home `tools/` 目录，激活已启用工具 |
| `index.ts` | tools 模块 re-export |
| `ask/tool.ts` | 向用户提问（阻塞等待回答） |
| `edit/tool.ts` | 文件编辑（含 hashline 模式） |
| `edit/edit-approval.ts` | 编辑审批流 |
| `edit/hashline.ts` | Hashline 编辑格式解析 |
...
| `supervisor-resources/constants.ts` | 工具侧资源路径常量 |
| `supervisor-resources/paths.ts` | 资源 URL 路径片段 |
...
| `supervisor-resources.test.ts` | 资源 URL 读写 |
...
SessionManager ──► SessionInputQueue（统一输入队列）
    │                      │
    ├─► SupervisorSessionRuntime ──► AgentHarness ──► LLM
    │         │
    │         └─► ExtensionRuntime ──► 扩展 / MCP / 打包工具
    │
    ├─► SupervisorDb (SQLite)
    │
