...
function externalizeSessionPaths(value: string): string {
  return value.replaceAll("@/", "${SV_SESSION_DIR}/");
}
...
export abstract class ExternalSessionRuntime implements ManagedSessionRuntime {
  ...
  protected readonly db: WecodeDb;
  ...
  protected constructor(options: { db: WecodeDb; session: Session; agent: Agent }) {
  ...
  protected sessionProjectId(): number {
    if (this.session.projectId == null) throw new Error("session has no project");
    ...
    try {
      ...
      const imageContent = images?.length
        ? await timedSessionStep(this.id, "resolvePromptImages", () =>
            resolveSessionPromptImages(this.sessionProjectId(), this.id, images),
          )
        : undefined;
      ...
      try {
        ...
        const externalMessageText = externalizeSessionPaths(message);
  ...
  async clear(): Promise<void> {
    ...
    await this._plugin?.clear().catch(() => {});
    this._plugin = null;
