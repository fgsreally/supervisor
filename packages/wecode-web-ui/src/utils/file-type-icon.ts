...
const FOLDER_NAME_ICONS: Record<string, string> = {
  ...
  composables: "vscode-icons:folder-type-component",
...
export function fileIconNameFromPath(path: string, isDirectory = false, isOpen = false): string {
  const base = getFileBaseName(path);
  const lower = base.toLowerCase();

  if (isDirectory || path.endsWith("/") || path.endsWith("\\")) {
    const folder = FOLDER_NAME_ICONS[lower];
    if (folder && isOpen) return `${folder}-opened`;
    return folder ?? (isOpen ? "vscode-icons:default-folder-opened" : "vscode-icons:default-folder");
  }
