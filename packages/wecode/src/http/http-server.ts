...
import { getWecodeAgentsRoot, isBuiltinAgent } from "../agent/index.js";
import { normalizeApiProtocol } from "../config/api-protocol.js";
import type { ApprovalResult, PluginEvent } from "../plugin/index.js";
...
import { listPendingApprovals } from "../plugin/runtime/index.js";
import {
...
import { ensureWecodePublicDir } from "../utils/wecode-home.js";
import { WecodeElysiaBuilder, type WecodeElysiaApp, type WecodeHttpContext } from "./elysia-app.js";
...
function collectRequestHeaders(c: WecodeHttpContext): Record<string, string> {
...
function emitSessionPluginEvent(
  manager: SessionManager,
  sessionId: number,
  event: PluginEvent,
): void {
  void manager.emitSessionPluginEvent(sessionId, event).catch(() => {});
}
...
export function createHttpServer(
  ...
  const app = new WecodeElysiaBuilder((c) => {
    ...
    if (
      ...
      return undefined;
    if (c.req.header("x-wecode-password") === password || c.req.query("password") === password)
      return undefined;
  ...
  app.get("/healthz", (c) =>
    c.json({
      ok: true,
      tunnelQuick,
      buildPreview: process.env.WECODE_BUILD_PREVIEW === "1",
    }),
  );
  app.post("/dev-preview/update", (c) => {
    if (process.env.WECODE_BUILD_PREVIEW !== "1") {
      return jsonError(c, 404, "build preview is not enabled");
    }
    setTimeout(() => process.exit(75), 100);
    return c.json({ ok: true, restarting: true });
  });
  app.get("/dev-preview/status", (c) => {
    const port = process.env.WECODE_BUILD_PORT ?? "3043";
    const path = resolve(tmpdir(), `wecode-build-preview-${port}.json`);
    const respond = (value: unknown) => {
      const response = c.json(value);
      response.headers.set("Access-Control-Allow-Origin", "*");
      return response;
    };
    try {
      return respond(JSON.parse(readFileSync(path, "utf8")));
    } catch {
      return respond({ phase: "空闲", progress: 0 });
    }
  });
  app.get("/auth/status", (c) =>
    c.json({
      required: Boolean(password),
      authenticated: !password || c.req.header("x-wecode-password") === password,
      tunnelQuick,
    }),
  );
  ...
  app.get("/settings/local-speech/models", (c) => {
    return c.json({
      selectedId: resolveLocalSpeechModel(readWecodeSettings().localSpeechModelId).id,
  ...
  app.patch("/settings", async (c) => {
    ...
    const saved = writeWecodeSettings(patch);
    const {
    ...
    try {
      if (providerName === "doubao") {
        ...
        if (result?.preset && result.preset !== settings.doubaoSpeechPreset) {
          writeWecodeSettings({ doubaoSpeechPreset: result.preset });
        }
  ...
  // POST /system/pick-directory — native folder dialog on the wecode host
  ...
  // POST /system/path-exists — check whether a path exists on the wecode host
  ...
  // GET /sessions/:id/log — structured session + plugin logs for UI
  ...
  app.get("/sessions/:id/assets/:scope/*", async (c) => {
    ...
    if (scope === "session" && session.projectId != null) {
      ...
      root = agent?.homeDir ?? join(getWecodeAgentsRoot(), String(session.agentId));
    }
  ...
  app.post("/sessions/:id/complete", async (c) => {
    try {
      const id = parseIntegerId(c.req.param("id"));
      if (id === null) return jsonError(c, 400, "invalid session id");
      if (manager.get(id)?.isBuiltin) return jsonError(c, 403, "WeCode 助手不能归档");
  ...
  app.post("/sessions/:id/services/wake", async (c) => {
    ...
    if (!session) return jsonError(c, 404, "not found");
    try {
      await manager.emitSessionPluginEvent(id, {
  ...
  app.post("/sessions/:id/prompt", async (c) => {
    ...
    emitSessionPluginEvent(manager, sessionId, {
      type: "http.request",
      method: "POST",
  ...
  app.get("/sessions/:id/commands", async (c) => {
    ...
    try {
      ...
      // Unrestorable sessions cannot expose plugin-owned commands.
    }
    try {
  ...
  // GET /session-events — SSE stream of status changes for the session list
  app.get("/session-events", async (c) => {
    ...
    const unsubscribe = manager.onAnySessionOutput((sessionId, event) => {
      if (event.type !== "session_status") return;
      void write({ type: "status", sessionId, event });
    });

    void write({ type: "connected" });
    c.req.raw.signal.addEventListener(
      "abort",
      () => {
        unsubscribe();
  ...
  // GET /sessions/:id/events — SSE stream of agent + wecode events
  ...
  app.patch("/sessions/:id/meta", async (c) => {
    ...
    try {
      ...
      if (manager.get(id)?.isBuiltin && fields.pinned === false) {
        return jsonError(c, 403, "WeCode 助手不能取消置顶");
      }
  ...
  // GET /sessions/:id/ui-menus — plugin-provided session/message actions
  ...
  // POST /sessions/:id/ui-menus/:menuId — execute a plugin UI action
  ...
  // GET /files/content?path=<path>  — read file; only ~/.pi/ and wecode agent homes are allowed
  app.get("/files/content", async (c) => {
    ...
    // Whitelist: wecode agent homes only
    const allowedRoots = [normalize(getWecodeAgentsRoot())];
  ...
  // POST /sessions/:id/approval-resolve — resolve pending plugin UI approval
  ...
  // GET /resources/global — global catalog (~/.pi/wecode/global/)
  ...
  // GET /agents/:id/plugins — builtins + user plugins with enabled flags
  app.get("/agents/:id/plugins", (c) => {
    const agentId = parseIntegerId(c.req.param("id"));
    if (agentId === null) return jsonError(c, 400, "invalid agent id");
    try {
      return c.json(manager.listAgentPlugins(agentId));
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : String(e);
      return jsonError(c, message.includes("not found") ? 404 : 500, message);
    }
  ...
  // PATCH /agents/:id/plugins/:resourceId — enable/disable (allowed for built-in agents)
  app.patch(
    "/agents/:id/plugins/:resourceId",
    async (c) => {
      const agentId = parseIntegerId(c.req.param("id"));
      ...
      try {
        ...
        const binding = manager.setAgentPluginEnabled(agentId, resourceId, body.enabled);
    ...
    },
    {
      ...
      detail: { summary: "Enable or disable an Agent plugin binding" },
    },
  );
  ...
  app.post("/agents/:id/resources", async (c) => {
    ...
    try {
      ...
      if (kind === "plugin") {
        await manager.reloadNativeAgentPlugins(agentId);
      } else {
  ...
  // ============ Plugin Management (global catalog) ============

  // GET /plugins — list plugins from resource catalog (exclude shipped builtins)
  app.get("/plugins", (c) => {
    try {
      const all = manager.resources.listResources("plugin");
  ...
  // POST /plugins/install — install plugin from source
  app.post(
    "/plugins/install",
    async (c) => {
    ...
    },
    {
      body: t.Object({
        source: t.String({
          minLength: 1,
          description: "npm:<spec>, git:<url>, or a local plugin directory",
        }),
      }),
      detail: { summary: "Install a plugin into the global resource catalog" },
    },
  );

  // POST /plugins/:id/update — re-fetch from package.json repository
  app.post("/plugins/:id/update", async (c) => {
    const id = c.req.param("id");
    try {
      const result = await manager.resources.updateResource("plugin", id);
      return c.json(result);
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : String(e);
      return jsonError(c, 400, message);
  ...
  app.post("/plugins/:id/uninstall", async (c) => {
    const id = c.req.param("id");
    try {
      await manager.resources.uninstallResource("plugin", id);
      return c.json({ ok: true });
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : String(e);
      return jsonError(c, 400, message);
  ...
  // POST /upload/public — upload a file into ~/.pi/wecode/public (avatars, etc.)
  ...
  app.get("/public/*", async (c) => {
    ...
    try {
      const root = ensureWecodePublicDir();
  ...
  app.post("/upload/public", async (c) => {
    try {
      ...
      const publicDir = ensureWecodePublicDir();
  ...
  app.get("/uploaded-icons/:filename", async (c) => {
    ...
    try {
      const publicFile = join(ensureWecodePublicDir(), filename);
      const legacyFile = join(getWecodeAgentsRoot(), "icons", filename);
      let content: Buffer;
      try {
