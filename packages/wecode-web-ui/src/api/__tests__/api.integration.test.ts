 ...
 * with the Wecode backend.
 *
 * Prerequisites:
 * - Wecode backend must be running on default port
 * - Or use VITE_API_BASE env var to point to test server
 */

describe("API Integration", () => {
  // Skip unless wecode is running (WECODE_INTEGRATION=1 npm run test)
  const itIfBackend = process.env.WECODE_INTEGRATION === "1" ? it : it.skip;
