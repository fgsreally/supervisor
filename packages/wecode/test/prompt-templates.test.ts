...
describe("Template expansion", () => {
  ...
  it("accepts the internal prompt trigger", () => {
    expect(expandPromptTemplate(">answer topic concise", [template])).toBe(
      "first=topic; second=concise; all=topic concise; tail=concise; one=topic",
    );
  });
