...
import type { LiveStatusPayload, LiveStatusPhase } from "wecode-native-bridge";

import { useSessionStore } from "@/store";
import { isNativeApp } from "./use-native-app";
import { translate as t } from "@/i18n";

let wecodeNative: typeof import("wecode-native-bridge").WecodeNative | null = null;
...
async function plugin() {
  ...
  if (!wecodeNative) {
    const mod = await import("wecode-native-bridge");
    wecodeNative = mod.WecodeNative;
  }
  return wecodeNative;
}
...
function buildPayload(): LiveStatusPayload | null {
  ...
  if (runningEntries.length === 1 && completed === 0) {
    return {
      ...
      title: truncateTitle(primaryRunning?.title || "Wecode", 16),
...
export async function endLiveStatus(sessionId: string): Promise<void> {
  applyTracked({
    sessionId,
    title: tracked.get(sessionId)?.title ?? "Wecode",
    subtitle: t("live.completed"),
    phase: "idle",
    running: false,
  });
    ...
    () => {
      for (const session of sessionStore.sessions) {
        ...
        applyTrackedState({
          ...
          title: session.title?.trim() || "Wecode",
