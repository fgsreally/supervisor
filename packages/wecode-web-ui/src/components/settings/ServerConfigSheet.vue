...
watch(open, (visible) => {
  if (!visible) return;
  if (props.editingId) {
    const editing = listWecodeInstances().find((item) => item.id === props.editingId) ?? null;
...
async function save() {
  ...
  try {
    ...
    if (backgroundEnabled.value) {
      await startBackgroundConnection("Wecode", t("mobile.connected"));
    } else {
