...
1. **Wecode 全局资源**：`~/.wecode/global/skills/`、`prompts/`、`plugins/`
...
同名时项目 skill 优先。npx 全局 skill 与 Wecode 自有全局 skill 一样，仍需绑定到 Agent 才会进入 Session。
...
`definePlugin` 风格的插件文件，通过 `jiti` 加载。
...
- Skills 可通过搜索（`GET /skills/search`）或直接粘贴链接安装到 Wecode 全局目录
