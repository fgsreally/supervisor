      ...
      for (const presetId of candidates) {
        try {
          ...
          if (presetId !== settings.doubaoSpeechPreset) {
            writeWecodeSettings({ doubaoSpeechPreset: presetId });
          }
          return;
        } catch (error) {
    ...
    return new Promise<void>((resolve, reject) => {
      const headers =
        this.provider === "doubao"
          ...
          : { Authorization: `Bearer ${qwenApiKey}`, "User-Agent": "wecode" };
...
function handleSessionMessage(
  ...
  if (message.type === "subscribe-statuses") {
    state.unsubscribe?.();
    state.subscribedSessionId = null;
    state.unsubscribe = manager.onAnySessionOutput((sessionId, event) => {
      if (event.type !== "session_status") return;
      sendJson(socket, { channel: "session", type: "status", sessionId, event });
    });
    sendJson(socket, { channel: "session", type: "connected" });
    return;
  }
