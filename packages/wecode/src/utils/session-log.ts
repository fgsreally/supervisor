...
function legacyPluginLogPath(projectId: string | number, sessionId: string | number): string {
...
/** Parse legacy plain-text plugin log lines into structured entries. */
function parseLegacyPluginLine(line: string): SessionLogEntry | null {
  const trimmed = line.trim();
  if (!trimmed) return null;
...
/** Read structured session logs (+ legacy plugin plaintext if present). */
export function readSessionLog(
  ...
  const legacy = legacyPluginLogPath(projectId, sessionId);
  if (existsSync(legacy)) {
    try {
      const text = readFileSync(legacy, "utf8");
      for (const line of text.split(/\r?\n/)) {
        const entry = parseLegacyPluginLine(line);
        if (entry && matchesFilter(entry, options)) entries.push(entry);
      }
    } catch (error) {
      writeLog(
