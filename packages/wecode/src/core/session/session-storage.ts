...
export class SQLiteSessionStorage implements SessionStorage {
  ...
  constructor(db: WecodeDb, sessionId: number) {
    this.db = db;
  ...
  async getHarnessMetadata(): Promise<WecodeHarnessMetadata> {
  ...
  async appendEntry(entry: SessionTreeEntry, options: AppendEntryOptions = {}): Promise<void> {
    ...
    const hiddenFlowMessage =
      ...
      queuedSource.startsWith("plugin:flow:");
