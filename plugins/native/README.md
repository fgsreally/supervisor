...
Wecode 插件：用 omp 的 Rust 原生库（`@oh-my-pi/pi-natives`）**覆盖**默认工具。
...
插件还会在 `write` / `edit` / `bash` / `ast_grep` 写盘后调用 `invalidateFsScanCache`，与 omp 的 grep/find 缓存一致。

## output-minimizer 与 native bash

- **native bash**：压缩在 `executeShell` 内由 **Rust minimizer** 完成（可通过 `OMP_MINIMIZER_*` 环境变量配置）。
- **默认 bash**：走 wecode 内置 **`wecode-output-minimizer`** 插件（TS 移植版）。

启用 `wecode-native` 后，output-minimizer 会自动跳过已由 Rust 压缩过的 bash 输出，避免重复处理。

## 与内置插件的关系

| 内置插件                      | 冲突                           |
| ----------------------------- | ------------------------------ |
| `wecode-ast-grep`         | 同名 `ast_grep`，后加载者覆盖  |
| `wecode-web`              | 同名 `web_fetch`，后加载者覆盖 |
| `wecode-output-minimizer` | 与 native bash 冗余，可关闭    |
...
Wecode 运行在 **Node**（>= 20.6），不需要 Bun。安装：

```bash
wecode plugins install ./plugins/native
```
