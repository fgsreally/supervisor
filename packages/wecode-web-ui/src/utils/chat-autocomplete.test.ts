...
describe("skill trigger", () => {
  it("uses dollar trigger for internal skills", () => {
    expect(getAutocompleteContext("$rev", 4)?.trigger).toBe("skill");
    const items = getAutocompleteSuggestions(
      { trigger: "skill", prefix: "$rev", replaceStart: 0, replaceEnd: 4 },
      {
        workspaceFiles: [],
...
describe("prompt trigger", () => {
  it("uses greater-than trigger for prompt templates", () => {
    expect(getAutocompleteContext(">sum", 4)?.trigger).toBe("prompt");
    const items = getAutocompleteSuggestions(
      { trigger: "prompt", prefix: ">sum", replaceStart: 0, replaceEnd: 4 },
      {
        workspaceFiles: [],
        skills: [],
    ...
    expect(items[0]?.label).toBe(">summarize");
  });
});
