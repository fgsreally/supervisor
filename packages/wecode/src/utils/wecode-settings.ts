...
export interface WecodeSettings {
  ...
  /** Optional SQLite path; project `.wecode/config.json` takes precedence. */
  dbPath?: string;
...
export function resolveDoubaoSpeechFromSettings(settings: WecodeSettings) {
...
export function isDoubaoSpeechConfigured(settings: WecodeSettings): boolean {
...
export function resolveSpeechRecognitionMode(settings: WecodeSettings): SpeechRecognitionMode {
  const mode = settings.speechRecognitionMode;
  if (mode === "qwen" || mode === "doubao" || mode === "local") return mode;
  return "local";
}

const DEFAULT_SETTINGS: WecodeSettings = {};
...
export function getWecodeSettingsPath(): string {
  return join(getWecodeHome(), "settings.json");
}

export function readWecodeSettings(): WecodeSettings {
  const path = getWecodeSettingsPath();
  if (!existsSync(path)) return { ...DEFAULT_SETTINGS };
  try {
    const parsed = JSON.parse(readFileSync(path, "utf-8")) as WecodeSettings;
...
export function writeWecodeSettings(patch: Partial<WecodeSettings>): WecodeSettings {
  const current = readWecodeSettings();
  const next: WecodeSettings & Record<string, unknown> = { ...current };
  delete next.doubaoSpeechAppId;
  delete next.doubaoSpeechAccessTokenEncrypted;
  for (const [key, value] of Object.entries(patch) as [keyof WecodeSettings, unknown][]) {
