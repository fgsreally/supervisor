...
import { SessionPluginHost } from "../../plugin/runtime/index.js";
...
import type { ApprovalRequest, ApprovalResult } from "../../plugin/index.js";
import { harnessAgentState, readHarnessTools } from "../agent/harness-compat.js";
import { loadSessionPlugins } from "./session-plugin-attach.js";
import {
...
export type SlashCommandSource = AgentResourceCommandSource | "plugin" | string;
...
export class SessionRuntime implements ManagedSessionRuntime {
  ...
  /** 与当前运行中 Agent 会话唯一绑定的 Plugin 实例。 */
  private _plugin: SessionPluginHost | null = null;
  ...
  constructor(options: SessionRuntimeOptions) {
    ...
    this.storage?.onEntryAppended((entry) => this._plugin?.handleStoredEntry(entry));

    this.harness.subscribe((event) => {
      void this.emit(event);
      void this._plugin?.handleHarnessEvent(event);
    });
  ...
  /**
   * Initialize the plugin runtime: load and setup all plugins.
   */
  async initPlugins(
    agentId: number,
    agentName: string,
    ...
    // The built-in WeCode assistant is deliberately global and has no project-scoped
    // artifact directory. Its regular Agent tools and resources still work, while
    // project/session plugins are skipped because their Context requires a project.
    if (session?.projectId == null) return;
    const plugin = await loadSessionPlugins({
      runtime: this,
      agentId,
      agentName,
      cwd,
    ...
    await activatePackagedTools(plugin, {
      cwd,
      sessionId: this.id,
      sessionDir,
    ...
    plugin.bindHarness(this.harness);
  }

  /** Keep harness permission cwd in sync after plugins change session cwd (e.g. worktree). */
  ...
  /** Tear down and reload plugin modules for the current Agent binding set. */
  async reloadPlugins(
    agentId: number,
    agentName: string,
    cwd: string,
    ...
    await this.initPlugins(agentId, agentName, cwd, db, manager, "restore");
  }

  /**
   * Collect all tools registered by plugins as AgentTool[].
   */
  collectPluginTools(): AgentTool[] {
    return this._plugin?.collectTools() ?? [];
  }
  ...
  async deactivatePlugin(pluginId: string): Promise<boolean> {
    return (await this._plugin?.unload(pluginId)) ?? false;
  }

  /**
   * Check if the current harness event carries an error condition
   * and forward it to plugins if so.
   */
  forwardErrorToPlugins(errorMessage: string, messageId?: string): void {
    this._plugin?.forwardError(errorMessage, messageId);
  }

  /**
   * Forward tool execution events to plugins.
   */
  forwardToolExecutionToPlugins(_event: {
    ...
    const ext = this._plugin;
    if (!ext) return;
    // Currently forwarded via harness tool_call/tool_result hooks in initPlugins
  }

  /** 清理与当前 Agent 绑定的 Plugin 和其他 Resource。 */
  ...
  async prompt(
    ...
    try {
      const session = this.getSession();
      if (images?.length && (!session || session.projectId == null)) {
        throw new Error("session has no project");
      }
      const imageContent = images?.length
        ? await resolveSessionPromptImages(session!.projectId!, this.id, images)
        : undefined;
  ...
  async steer(message: string, images?: SessionPromptImage[]): Promise<void> {
    if (images?.length) {
      ...
      const session = this.getSession();
      if (!session || session.projectId == null) throw new Error("session has no project");
      const imageContent = await resolveSessionPromptImages(session.projectId, this.id, images);
  ...
  async setTools(tools: AgentTool[], activeToolNames?: string[]): Promise<void> {
    const pluginTools = this._plugin?.wrapTools(tools) ?? tools;
    const effectiveTools = this.permissionConfig
      ? pluginTools.map((tool) => this.wrapPermissionTool(tool))
      : pluginTools;
    const toolServices = this._plugin?.services.tools;
  ...
  async syncActiveTools(): Promise<void> {
    ...
    const active =
      this._plugin?.services.tools.filterActiveNames(known) ??
      this._plugin?.listActiveToolNames(known) ??
      known;
  ...
  getSlashCommands(): SlashCommandInfo[] {
    ...
    for (const command of this._plugin?.getAllCommands() ?? []) {
      commands.set(command.name, {
        name: command.name,
        description: command.description,
  ...
  async executeSlashCommand(name: string, args: string): Promise<void> {
    if (!this._plugin) throw new Error("Plugin runtime is unavailable");
    const raw = `/${name}${args ? ` ${args}` : ""}`;
    const commandSource = this.getSlashCommands().find(
      (command) => command.name === name.replace(/^\//, ""),
    )?.source;
    const result = await this._plugin.executeCommand(name, args);
