...
import { ChevronLeft, ChevronRight, FolderTree, Loader2, RefreshCw } from "lucide-vue-next";
import {
...
const treePaneWidth = ref(Number(localStorage.getItem("wecode:file-tree-width")) || 220);
...
function startTreeResize(event: PointerEvent) {
  ...
  const onUp = () => {
    localStorage.setItem("wecode:file-tree-width", String(treePaneWidth.value));
    stopTreeResize?.();
  };
