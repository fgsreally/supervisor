...
@objc(WecodeNativePlugin)
public class WecodeNativePlugin: CAPPlugin, CAPBridgedPlugin {
    public let identifier = "WecodeNativePlugin"
    public let jsName = "WecodeNative"
    ...
    private static weak var instance: WecodeNativePlugin?
    private var scanCall: CAPPluginCall?

    override public func load() {
        super.load()
        WecodeNativePlugin.instance = self
    }

    @objc public static func ingestSharedFiles(_ urls: [URL]) {
        ShareQueue.enqueue(ShareQueue.copyToCache(urls))
        notifyShareReceived()
    }

    @objc public static func notifyShareReceived() {
        guard let instance, let payload = ShareQueue.toJSObject() else { return }
        instance.notifyListeners("shareReceived", data: payload)
    }
    ...
    @objc func getPendingShare(_ call: CAPPluginCall) {
        if let payload = ShareQueue.toJSObject() {
