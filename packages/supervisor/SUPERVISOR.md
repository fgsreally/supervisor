...
| `internal-agents.ts` | 注册随包 Agent 和内置助手；`agents.is_internal` 控制是否允许建立用户 Session，内置 skill 通过数据库绑定 |
...
| `schema.ts`                     | 对外再导出 `Type` / `Static`（扩展请从本包引入，勿直接依赖 typebox）    |
| `extension.ts`                  | 会话级 `Extension`：加载扩展、Harness 事件映射、工具适配与包装          |
| `loader.ts`                     | 扫描目录、动态加载扩展模块（jiti）                                      |
| `extension-installer.ts`        | 从 npm、Git 或本地路径安装扩展到全局资源目录                            |
| `extension-source.ts`           | 解析 npm、Git 和本地扩展来源                                            |
| `runtime.ts`                    | **ExtensionRuntime**：扩展注册表、工具注册、生命周期、内置扩展          |
| `extension-deps.ts`             | 将 SessionManager、数据库和 SessionRuntime 连接为 Context 内部能力      |
| `extension-entry.ts`            | 扩展入口解析与校验                                                      |
| `extension-session-services.ts` | 会话级服务：工具守卫、Turn 注入、审批                                   |
| `tool-policy.ts`                | 工具启用/禁用策略                                                       |
| `tool-probe.ts`                 | 干跑 `setup()` 探测扩展会注册哪些工具                                   |
...
| `agent-resources.ts`  | 汇总 Agent 已绑定资源与最终工具列表                    |
