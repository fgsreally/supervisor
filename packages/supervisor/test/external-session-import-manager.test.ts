...
describe("external session import", () => {
  it("returns imported history without waiting for project parsing", async () => {
    testCwd = join(tmpdir(), `supervisor-import-test-${Date.now()}`);
    mkdirSync(testCwd, { recursive: true });
    const db = new SupervisorDb(join(testCwd, "test.db"));
  ...
  it("starts an imported external runtime without session extensions", async () => {
    testCwd = join(tmpdir(), `supervisor-import-test-${Date.now()}`);
    mkdirSync(testCwd, { recursive: true });
    const db = new SupervisorDb(join(testCwd, "test.db"));
    db.insertAgent({ name: "Codex", backend_type: "codex" });
    ...
    const attach = vi.spyOn(manager as any, "attachExternalSessionExtensions");
    vi.spyOn(manager as any, "createExternalRuntime").mockResolvedValue({});

    await manager.importExternalSession({
      backend: "codex",
      externalSessionId: "external-without-extensions",
    });
  ...
  it("imports a Supervisor worktree conversation at the project root", async () => {
    testCwd = join(tmpdir(), `supervisor-import-test-${Date.now()}`);
    importedCwd = join(testCwd, ".supervisor", "worktrees", "148");
