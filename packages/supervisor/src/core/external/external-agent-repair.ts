...
export const ExternalAgentRepairResultSchema = Type.Object({
  ...
  command: Type.Optional(Type.String({ description: "A complete executable path or command name if the launch command must be updated" })),
  args: Type.Optional(
    Type.Array(Type.String(), { description: "A complete argument list if launch arguments must be updated" }),
  ),
});
