...
export async function assertCanReadSession(ctx: ExtensionContext, sessionId: number): Promise<void> {
...
async function readAgentResource(
	...
	if (parsed.resource === "summary" || parsed.resource === "") {
		const tools = ctx.tools.list().map((tool) => ({
			name: tool.name,
		...
		return {
			...
			packagedTools: listEnabledPackagedToolIds(getAgentHomeDir(agentId)),
			tools,
		};
	}
	...
	if (parsed.resource.startsWith("tools/")) {
		...
		return {
			agentId,
			toolId,
			enabled: listEnabledPackagedToolIds(getAgentHomeDir(agentId)).includes(toolId as never),
		};
	}
...
export async function readSupervisorResource(
	...
	if (parsed.kind === "session" && parsed.sessionId !== undefined) {
		...
		return readSessionResource(sqlite, parsed.sessionId, parsed.resource, limit);
	}
