...
import { escapeHtml } from "@/utils/code-highlight";
...
function createTerminalRenderer(): Renderer {
  ...
  renderer.code = ({ text, lang }) => {
    ...
    const index = markdownCodeBlocks.push({ text, lang, language }) - 1;
    return `<!--shiki-code:${index}-->`;
  };
...
const html = ref("");
let renderVersion = 0;
let markdownCodeBlocks: Array<{ text: string; lang?: string; language: string }> = [];
async function renderMarkdown() {
  ...
  let rendered = typeof raw === "string" ? raw : "";
  const blocks = await Promise.all(markdownCodeBlocks.map(async (block) => {
    const highlighted = await highlightCodeWithShiki(block.text, block.lang, isDark.value);
    return highlighted.replace(/^<pre[^>]*>/, `<pre class="md-term-pre"${block.language}>`);
  }));
  blocks.forEach((block, index) => { rendered = rendered.replace(`<!--shiki-code:${index}-->`, block); });
  if (version === renderVersion) html.value = rendered;
}
watch(() => [props.content, variant.value, isDark.value] as const, () => void renderMarkdown(), { immediate: true });
...
function onContentClick(event: MouseEvent) {
  ...
  window.dispatchEvent(
    new CustomEvent("wecode:open-file", { detail: { path: decodeURIComponent(href) } }),
  );
}
</script>
