...
/** Wecode runtime dirs must never appear in the session file tree / change list. */
export function isWecodeRuntimePath(path: string): boolean {
  const normalized = path.replace(/\\/g, "/").replace(/^\.\//, "");
  return (
    normalized === ".wecode" ||
    normalized.startsWith(".wecode/") ||
    normalized.split("/").includes(".wecode") ||
...
export function buildSessionFileTree(
  ...
  const paths = new Set(
    entries
      ...
      .filter((path) => !isWecodeRuntimePath(path)),
  );
  for (const file of changedFiles ?? []) {
    if (file?.path && file.status === "deleted") {
      const path = file.path.replace(/\\/g, "/");
      if (!isWecodeRuntimePath(path)) paths.add(path);
    }
  }
