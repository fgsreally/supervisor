    ...
    <div class="chat-workspace">
      ...
      >
        ...
        <FloatingPreviewOrb
          ...
          :storage-key="`wecode:preview-orb:${session.id}`"
        ...
        <FloatingPreviewOrb
          ...
          :container-ref="conversationHostRef"
          :storage-key="`wecode:bash-orb:${session.id}`"
...
import { fileBasename, isWecodeRuntimePath } from "../utils/session-file-tree";
...
import {
  exampleInitialEntries,
  getExampleBranches,
  getExampleComposerState,
  isExampleSession,
} from "@/examples";
...
const sessionChangedFiles = computed<SessionChangedFileView[]>(() => {
  ...
  return raw
    .filter((file) => file?.path && !isWecodeRuntimePath(file.path))
...
const { width: sidePanelWidth, startResize: startSidePanelResize } = useResizableWidth({
  ...
  minWidth: 280,
  maxWidth: 560,
  storageKey: "wecode:chat-side-panel-width",
  direction: "rtl",
});
...
onMounted(() => {
  window.addEventListener("wecode:open-file", onOpenFileEvent);
});
...
async function loadSessionMessages(sessionId: string) {
  ...
  try {
    if (isExampleSession(sessionId)) {
      const composer = getExampleComposerState(sessionId);
      ...
      suggestedQuestions.value = composer.suggestions;
      queuedInputs.value = composer.queuedInputs;
...
onBeforeUnmount(() => {
  ...
  window.removeEventListener("wecode:open-file", onOpenFileEvent);
  stopStreaming();
  void sessionDeviceSync.flush();
...
const sessionStatus = computed(
  () => sessionStore.getSessionById(props.session.id)?.status ?? props.session.status,
);

const headerStatusKey = computed(() => {
  if (pendingAsk.value) return "blocked";
...
async function consumePendingShare() {
  await nextTick();
  await attachPendingShareToInput((file) => inputPanelRef.value?.addPendingFile(file));
}
...
async function sendStreamReply(
  ...
  sessionStore.setSessionStatus(props.session.id, "running");
  ...
  streamCleanup = api.promptSession(
    ...
    (err) => {
      ...
      showUiMessage(t("chat.sendFailed"), "error");
      inputPanelRef.value?.restoreFailedSend({
        text: userText,
        images,
        pastedTexts,
        attachments,
      });
      // Failed sends belong in the composer, not in conversation history.
      chatEntries.value = chatEntries.value.filter(
        (entry) => entry.id !== assistantId && entry.id !== userEntry?.id,
      );
      isStreaming.value = false;
      streamingAssistantId.value = null;
      streamCleanup = null;
...
.model-picker-sheet header button:hover,
.model-picker-sheet header button:focus-visible {
  color: var(--app-text-primary);
  background: var(--app-hover);
}
