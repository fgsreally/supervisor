	...
	switch (command) {
		...
		case "extensions": {
			...
			if (subCmd === "list") {
				...
				if (infos.length === 0) {
					console.log("No extensions installed in global catalog.");
				} else {
					for (const info of infos) {
						const name = info.name ?? info.fileName;
						const ver = info.version ? ` v${info.version}` : "";
						console.log(`  ${info.id}  ${name}${ver}  [${info.entryPath}]`);
					}
				}
				db.close();
				break;
			...
			if (subCmd === "unlink") {
				...
				console.log(`Unlinked ${id} from agent ${agentId}`);
				db.close();
				break;
			}
			...
			const agentId = cmdArgs[1];
			if (!agentId) {
				console.error("Usage: pi-supervisor extensions <agent-list|agent-install|agent-remove|install|uninstall|list|link|unlink> ...");
				process.exit(1);
			}
			const agentHomeDir = getAgentHomeDir(agentId);
			...
			if (subCmd === "agent-install") {
				...
				const linked = installExtensionToAgentDir(agentHomeDir, source);
				console.log(`Linked extension ${linked.id} → ${linked.path}`);
				db.close();
				break;
			}
			if (subCmd === "agent-remove") {
				...
				removeExtensionFromAgentDir(agentHomeDir, id);
				console.log(`Removed extension ${id} from ${agentExtDir}`);
				db.close();
				break;
			}
		...
		case "providers": {
			...
			if (subCmd === "list") {
				...
				if (existing) {
					console.log(`Provider "${id}" already exists. Use \`providers set-key\` to update the key.`);
					db.close();
					break;
				}
