...
import { defineAgentPlugin } from "../src/plugin/index.js";
import {
  AgentPluginRuntime,
  SessionPluginHost,
  createEventBus,
...
describe("AgentPluginRuntime", () => {
  it("runs Agent setup once and session.setup once per attached Session with a reason", async () => {
    const first = createPluginTestContext(options(1));
    const second = createPluginTestContext(options(2));
    const firstHost = new SessionPluginHost(first);
    const secondHost = new SessionPluginHost(second);
    const setup = vi.fn();
    const sessionSetup = vi.fn();
    const cleanup = vi.fn();
    const runtime = new AgentPluginRuntime(7, first);
  ...
  it("keeps registrations made during session.setup scoped to that Session", async () => {
    const first = createPluginTestContext(options(11));
    const second = createPluginTestContext(options(12));
    const firstHost = new SessionPluginHost(first);
    const secondHost = new SessionPluginHost(second);
    const runtime = new AgentPluginRuntime(7, first);
