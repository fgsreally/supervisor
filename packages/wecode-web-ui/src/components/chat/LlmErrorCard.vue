  ...
  <div class="llm-error-row flex justify-start items-start gap-2">
    ...
    <div class="max-w-[75%] flex flex-col items-start min-w-0 w-full">
      <div class="llm-error-card">
        <div class="llm-error-card__title">{{ t("chat.message.replyFailed") }}</div>
        ...
        <button
          ...
          @click="$emit('retry')"
        >
          <Loader2 v-if="retrying" class="w-3.5 h-3.5 animate-spin" /><RefreshCw
            v-else
            class="w-3.5 h-3.5"
          />{{ retrying ? t("chat.message.retrying") : t("chat.message.retry") }}
        </button>
      </div>
    </div>
  </div>
