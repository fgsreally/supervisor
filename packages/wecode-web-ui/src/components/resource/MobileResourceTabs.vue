<template>
  <div class="bg-white border-y border-gray-200/80">
    ...
    <div
      v-if="loading && tab !== 'config' && tab !== 'system' && tab !== 'plugins'"
    ...
    <div v-else-if="tab === 'plugins'" class="min-h-[20rem]">
      <AgentPluginsPanel :agent-id="agentId" />
    </div>
...
import AgentPluginsPanel from "../agent/AgentPluginsPanel.vue";
...
const tabs = computed(() => [
  ...
  { id: "plugins" as const, label: t("resource.plugins") },
