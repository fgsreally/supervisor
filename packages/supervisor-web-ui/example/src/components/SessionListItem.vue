<template>
  ...
  >
    ...
    />
    <div v-if="isChild" class="absolute top-0 h-1/2 w-px session-tree-line" :style="treeLineStyle" />
    <div v-if="isChild && !isLastChild" class="absolute top-1/2 bottom-0 w-px session-tree-line" :style="treeLineStyle" />
    <div v-if="isChild" class="absolute top-1/2 h-px session-tree-line" :style="treeBranchStyle" />
    <div v-if="isChild" class="absolute top-[calc(50%-3px)] w-1.5 h-1.5 rounded-full" :style="treeDotStyle" />
    ...
    <div class="flex items-center gap-3 relative">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-baseline justify-between gap-2">
          ...
          <span class="text-[10px] shrink-0 session-time">{{ formatListTime(session.lastActiveAt) }}</span>
        </div>
...
const preview = computed(() =>
  props.mode === 'contacts' ? props.session.meta.description ?? props.session.id : props.session.lastMessagePreview,
)
