...
import {
  ...
  ShadowResultSchema,
} from "../src/plugin/builtin/shadow/protocol.js";
...
describe("shadow Watson submit_result protocol", () => {
  ...
  it("preserves plugin fields only for info results", () => {
    ...
    ).toMatchObject({ level: "info", plugins: { commitMessage: "checkpoint" } });
    expect(
      normalizeShadowSubmitResult(
  ...
  it("injects AGENTS.md from the session working directory", () => {
    const cwd = mkdtempSync(join(tmpdir(), "wecode-shadow-context-"));
    try {
