...
import { createSkillPlugin } from "../src/plugin/index.js";
import type { PluginContext, ToolDefinition } from "../src/plugin/types.js";

describe("skill plugin", () => {
  it("通过插件注册 skill 工具并转发资源操作", async () => {
    ...
    await createSkillPlugin(resource).setup(context);
  ...
  it("没有任何 skill 时不注册工具", async () => {
    ...
    const context = { agent: { registerTool } } as unknown as PluginContext;
