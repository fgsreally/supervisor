...
每个 Session 都有自己的专属目录，由 Wecode 分配，不等同于 Agent 的工作目录（`cwd`）。目录位置由 Wecode 内部管理，常用子目录包括：
...
Wecode 自己控制的文件工具（`read`、`ls`、`grep`、`find` 等）使用逻辑路径：
...
Shell、Python、JavaScript、eval，以及 Wecode 无法直接控制的外部程序使用环境变量：

```text
SV_SESSION_DIR/attachments/input.csv
```

外部 Agent 进程启动时会注入 `SV_SESSION_DIR`。外部提示词中的 `@/` 路径会转换为 `${SV_SESSION_DIR}/` 形式；Wecode 自己的原生 Agent 仍使用 `@/`。
