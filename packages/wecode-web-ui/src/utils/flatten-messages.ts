...
export function buildDisplayGroups(
  ...
  const conclusionOnly = options.collapseConclusionOnly ?? false;
  const shouldSplitAssistantMessages = splitAssistantMessages || conclusionOnly;
  ...
  for (const entry of entries) {
    ...
    if (entry.type === "message") {
      if (shouldSplitAssistantMessages) {
        flushAssistant();
        current = {
          id: entry.id,
          type: "grouped_assistant",
...
function processTurnForConclusionSplit(
  ...
  const source = groups.at(-1)!;
  const createdAt = groups.find((group) => group.createdAt != null)?.createdAt;
  const endedAt = [...groups].reverse().find((group) => group.endedAt != null)?.endedAt;
  return [
    {
      ...source,
      createdAt,
      endedAt,
      pieces: compactAssistantPieces(groups.flatMap((group) => group.pieces)),
      assets: groups.flatMap((group) => group.assets),
      usage: groups.at(-1)?.usage ?? groups[0]?.usage,
    },
  ];
}
