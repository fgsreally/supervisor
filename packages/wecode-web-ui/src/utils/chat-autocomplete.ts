...
export type AutocompleteTrigger = "at" | "atat" | "slash" | "skill" | "prompt";
...
function extractPromptPrefix(textBeforeCursor: string): string | null {
  for (let i = textBeforeCursor.length - 1; i >= 0; i--) {
    const ch = textBeforeCursor[i] ?? "";
    if (ch === ">" && isTokenStart(textBeforeCursor, i)) {
      const token = textBeforeCursor.slice(i);
...
export function getAutocompleteContext(
  ...
  const promptPrefix = extractPromptPrefix(textBeforeCursor);
  if (promptPrefix) {
    return {
      trigger: "prompt",
      prefix: promptPrefix,
      replaceStart: cursor - promptPrefix.length,
      replaceEnd: cursor,
    };
  }
...
export function getAutocompleteSuggestions(
  ...
  if (context.trigger === "prompt") {
    return fuzzyFilter(options.prompts, query, (prompt) => prompt.name)
      .slice(0, 12)
      .map((prompt) => ({
...
export function applyAutocompleteCompletion(
  ...
  if (context.trigger === "slash" || context.trigger === "skill" || context.trigger === "prompt") {
    const trigger = context.trigger === "slash" ? "/" : context.trigger === "skill" ? "$" : ">";
