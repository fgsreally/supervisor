...
export class AgentResource {
  ...
  private expandSkillCommand(message: string): string {
    ...
    if (!token.startsWith("/") && !token.startsWith("$")) return message;
    const skillName = token.startsWith("/skill:")
      ? token.slice(7)
      : token.startsWith("$")
        ? token.slice(1)
        : token.slice(1);
