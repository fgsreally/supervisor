        ...
        <template v-else>
          ...
          <div class="chat-input-editor-wrap flex-1 min-h-0 relative">
            ...
            <ChatComposer
              ...
              @remove-image="removePendingImageByPlaceholder"
            />
          </div>
          <ChatInputToolbar
            :voice="voice"
...
const imageCatalog = computed(() =>
  Object.fromEntries(
    pendingImages.value.map((item) => [
      item.placeholder,
      { previewUrl: item.previewUrl, name: item.name },
    ]),
  ),
);
...
const skillTrigger = computed<"slash" | "dollar">(() =>
  isExternalAgent.value ? "slash" : "dollar",
);
...
const { height: panelHeight, startResize } = useResizableHeight({
  ...
  storageKey: "wecode-chat-input-height-v3",
});
...
function removePendingImageByPlaceholder(placeholder: string) {
  const item = pendingImages.value.find((image) => image.placeholder === placeholder);
  if (item) removePendingImage(item.id);
}
...
async function restoreFailedSend(payload: ChatSendPayload) {
  text.value = text.value ? `${payload.text}\n${text.value}` : payload.text;
  restorePastedTexts(
    [...payload.pastedTexts, ...Object.values(pastedTexts.value)],
    [...payload.attachments, ...attachments.value],
  );
  // Let the earlier clear update settle before restoring the image catalog.
  await nextTick();
  pendingImages.value = [
    ...payload.images.map((image) => ({
      ...image,
      previewUrl: props.sessionId
        ? api.sessionMediaUrl(props.sessionId, image.mediaId)
        : image.previewUrl,
    })),
    ...pendingImages.value,
  ];
