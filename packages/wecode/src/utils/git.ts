...
const WECODE_GITIGNORE_ENTRY = ".wecode/";

const execGit = promisify(execFile);

/** Trailer appended to every commit created by wecode. */
export const WECODE_COMMIT_TRAILER = "Wecode: true";

/** Ensure commit message carries the wecode (sv) marker. */
export function withWecodeCommitMarker(message: string): string {
  const trimmed = message.replace(/\s+$/u, "").trim() || "wecode: changes";
  if (/(?:^|\n)(?:Wecode|Sv):\s*true\b/iu.test(trimmed)) return trimmed;
  return `${trimmed}\n\n${WECODE_COMMIT_TRAILER}`;
}
...
function ensureWecodeGitignored(repoRoot: string): void {
  ...
  const alreadyIgnored = lines.some((line) => {
    ...
    return (
      trimmed === WECODE_GITIGNORE_ENTRY ||
      trimmed === ".wecode" ||
      trimmed === ".supervisor/" ||
      trimmed === ".supervisor"
    );
  ...
  if (!alreadyIgnored) {
    ...
    writeFileSync(gitignorePath, `${prefix}${WECODE_GITIGNORE_ENTRY}\n`, "utf8");
  }

  let tracked = "";
  try {
    tracked = runGitSync(repoRoot, ["ls-files", ".wecode"]);
  } catch {
    tracked = "";
  }
  ...
  try {
    runGitSync(repoRoot, ["rm", "-r", "--cached", "-f", "--ignore-unmatch", ".wecode"]);
  } catch {
    return;
  }
  try {
    runGitSync(repoRoot, [
      "commit",
      "-m",
      withWecodeCommitMarker("chore: stop tracking .wecode worktrees"),
    ]);
  } catch {
 ...
 * under wecode/), we still `git init` a nested repo at the project path
...
export function ensureProjectGitRootSync(projectCwd: string): string {
  ...
  try {
    ...
    runGitSync(root, ["commit", "--allow-empty", "-m", withWecodeCommitMarker("wecode: init project")]);
  }
  ensureWecodeGitignored(root);
  return root;
}
...
export function sessionBranchName(sessionId: string): string {
  return `wecode/session-${sessionId.slice(0, 8)}`;
}

export function sessionWorktreePath(repoRoot: string, sessionId: string): string {
  const next = join(repoRoot, ".wecode", "worktrees", sessionId);
  const legacy = join(repoRoot, ".supervisor", "worktrees", sessionId);
...
export async function syncSessionWorktree(repoRoot: string, worktreePath: string): Promise<string> {
  ...
  await runGit(worktreePath, [
    ...
    target,
    "-m",
    withWecodeCommitMarker(`Sync ${target}`),
  ]);
  return target;
}
...
export async function listLocalBranches(repoRoot: string): Promise<string[]> {
  try {
    ...
    const local = refs
      ...
      .filter((name) => !name.startsWith("wecode/session-") && !name.startsWith("pi/session-"));
    const localSet = new Set(local);
    const remote = refs
      .filter((ref) => ref.startsWith("refs/remotes/") && !ref.endsWith("/HEAD"))
      .map((ref) => ref.slice("refs/remotes/".length))
      .filter((name) => {
        const branch = name.slice(name.indexOf("/") + 1);
        return !branch.startsWith("wecode/session-") && !branch.startsWith("pi/session-");
      })
...
export async function createSessionWorktree(
  ...
  mkdirSync(join(repoRoot, ".wecode", "worktrees"), { recursive: true });
  const startBranch = await getCurrentBranch(repoRoot);

  // Session restore / plugin reload reruns session.setup; reuse an existing worktree.
...
export async function commitAll(
  ...
  const marked = withWecodeCommitMarker(message);
...
export async function createGitSnapshot(cwd: string): Promise<string | null> {
  ...
  const directory = mkdtempSync(join(tmpdir(), "wecode-git-index-"));
  ...
  try {
    ...
    const { stdout: snapshot } = await runGit(
      cwd,
      ["commit-tree", tree, "-p", head, "-m", "wecode worktree snapshot"],
      env,
    );
...
export async function mergeSessionBranch(
  ...
  await runGit(repoRoot, ["merge", "--no-ff", branch, "-m", withWecodeCommitMarker(`Merge ${branch}`)]);
}

export interface WecodeCommitInfo {
  hash: string;
  shortHash: string;
  subject: string;
  author: string;
...
/** List wecode-marked commits in [sinceMs, untilMs) across all refs. */
export async function listWecodeCommitsBetween(
  cwd: string,
  sinceMs: number,
  untilMs: number,
): Promise<WecodeCommitInfo[]> {
  ...
  const commits: WecodeCommitInfo[] = [];
...
function isWecodeWorktreePath(worktreePath: string): boolean {
  const normalized = worktreePath.replace(/\\/g, "/");
  return (
    normalized.includes("/.wecode/worktrees/") || normalized.includes("/.supervisor/worktrees/")
  );
}
...
export async function removeSessionWorktree(
  ...
  if (existsSync(worktreePath)) {
    ...
    if (isWecodeWorktreePath(worktreePath)) {
      try {
