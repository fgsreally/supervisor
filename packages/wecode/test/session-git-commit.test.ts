...
import { WecodeDb } from "../src/db/db.js";
import {
  commitGitSnapshot,
  createGitSnapshot,
  getGitHead,
...
beforeEach(() => {
  tmpDir = join(tmpdir(), `wecode-git-commit-${Date.now()}`);
...
describe("wecode: explicit commit", () => {
  ...
  it("commitSessionChanges updates meta.git.lastCommit", async () => {
    ...
    expect(commit?.message).toContain("Wecode: true");
