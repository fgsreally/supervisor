<template>
  ...
  >
      ...
      <div class="settings-content">
        <section v-if="showServices" class="settings-card">
          <h2 class="settings-card-title-row">
            <span class="settings-watson-title"><WatsonIcon />{{ t("settings.watson") }}</span>
            <small>{{ t("settings.watsonHint") }}</small>
          </h2>
          <label class="settings-field">
            <span>{{ t("settings.assistantModel") }}</span>
            <ModelTreeSelect
          ...
          <div v-if="showDiagnostics" class="service-list">
            <div class="service-row">
              <div class="service-copy">
                ...
                <span>{{ t("settings.watsonHint") }}</span>
              </div>
              <button class="configure-button" type="button" @click="loadLog('watson')">
                <ScrollText class="h-4 w-4" />{{ t("settings.viewLog") }}
              </button>
            </div>
          </div>
        </section>
        ...
        <section class="settings-card">
          <h2>{{ mode === "diagnostics" ? t("settings.logs") : t("settings.system") }}</h2>
          <div class="service-list">
            <div v-if="mode === 'diagnostics'" class="service-row">
              <div class="service-copy">
                <strong>{{ t("settings.watsonLogs") }}</strong>
                <span>{{ t("settings.watsonHint") }}</span>
              </div>
              <button class="configure-button" type="button" @click="loadLog('watson')">
                <ScrollText class="h-4 w-4" />{{ t("settings.viewLog") }}
            ...
            <div class="service-row">
              <div class="service-copy">
                <strong>{{
                  mode === "diagnostics" ? t("settings.systemLogs") : t("settings.logs")
                }}</strong>
        ...
        <section v-if="showDiagnostics" class="settings-card">
          ...
          <div class="service-list">
            ...
            <div class="service-row service-row--font-scale">
              ...
              <div class="font-scale-segment" role="group" :aria-label="t('settings.fontSize')">
                <button
        ...
        <section v-if="showServices" class="settings-card">
          ...
          <label class="settings-field">
            <span>{{ t("settings.startupMode") }}</span>
            <select v-model="form.browserMode">
              <option value="headless">{{ t("settings.headless") }}</option>
              <option value="headed">{{ t("settings.headed") }}</option>
            </select>
          </label>
        </section>

        <section v-if="showServices" class="settings-card">
          <h2>{{ t("settings.speech") }}</h2>
          <label class="settings-field">
            <span>{{ t("settings.currentService") }}</span>
            <select v-model="form.speechRecognitionMode">
              <option value="local">{{ t("settings.localSpeech") }}</option>
              <option value="qwen">{{ t("settings.qwenSpeech") }}</option>
              <option value="doubao">{{ t("settings.doubaoSpeech") }}</option>
            </select>
          </label>
          <div class="service-list">
            <div v-for="service in speechServices" :key="service.id" class="service-row">
              ...
              <span
                ...
                :title="service.configured ? t('settings.configured') : undefined"
              >
                <Check
                  v-if="service.configured"
                  class="h-5 w-5"
                  :aria-label="t('settings.configured')"
                />
              </span>
              <button class="configure-button" type="button" @click="openService(service.id)">
                <Settings2 class="h-4 w-4" />{{ t("settings.configure") }}
              </button>
            </div>
          </div>
        </section>
    ...
    <ResponsiveDialog :open="!!activeLog" width="xl" size="tall" @close="activeLog = null">
      <template #header>
        <div class="m-drawer__heading">
          <span class="m-drawer__title settings-log-title">
            <WatsonIcon v-if="activeLog === 'watson'" />
            {{ activeLog === "watson" ? t("settings.watsonLogs") : t("settings.systemLogs") }}
          </span>
          <p class="m-drawer__desc">
            {{
              activeLogFiles.length
                ? t("settings.logSummary", { count: activeLogFiles.length })
                : t("settings.liveLogs")
            }}
          </p>
        </div>
...
import ModelTreeSelect, { type ModelTreeGroup } from "../provider/ModelTreeSelect.vue";
...
import LogViewer from "@/components/base/LogViewer.vue";
...
function onLocaleChange(event: Event): void {
  ...
  if (value === "en" || value === "zh-CN") setLocale(value as AppLocale);
}
...
const pageTitle = computed(() => {
  if (mode.value === "services") return t("settings.services");
  if (mode.value === "diagnostics") return t("settings.diagnostics");
  return t("settings.page");
});
...
const fontScaleOptions = computed<{ value: AppFontScale; label: string }[]>(() => [
  { value: "small", label: t("settings.fontSize.small") },
  { value: "standard", label: t("settings.fontSize.standard") },
  { value: "large", label: t("settings.fontSize.large") },
]);

const form = reactive({
  browserMode: "headless" as "headless" | "headed",
  webSearchProvider: "duckduckgo" as NonNullable<WecodeSettings["webSearchProvider"]>,
  webFetchProvider: "native" as NonNullable<WecodeSettings["webFetchProvider"]>,
...
const draftDoubaoSpeechPreset =
  ref<NonNullable<WecodeSettings["doubaoSpeechPreset"]>>("2.0-duration");
...
function buildFeatureModels(): NonNullable<WecodeSettings["featureModels"]> {
  const next: NonNullable<WecodeSettings["featureModels"]> = {};
  const ref = parseFeatureKey(featureModelKeys.assistant);
  if (ref) next.assistant = ref;
  return next;
}

function mainPatch(): Partial<WecodeSettings> {
  return {
...
async function saveService() {
  ...
  try {
    ...
    apply(await updateWecodeSettings(patch));
    closeService();
    failed.value = false;
...
async function saveMain() {
  ...
  message.value = "";
  try {
    apply(await updateWecodeSettings(mainPatch()));
    failed.value = false;
    message.value = t("settings.saved");
    showUiMessage(t("settings.settingsSaved"), "success");
  } catch (error) {
