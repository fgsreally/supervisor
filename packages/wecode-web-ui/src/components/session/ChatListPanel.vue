  ...
  >
    ...
    >
      ...
      <button
        ...
        @click="emit('collapse')"
      >
        <PanelLeftClose class="h-5 w-5" stroke-width="1.75" />
      </button>
    </div>
    ...
    >
      <template v-if="query.trim()">
        <div v-if="searching" class="chat-search-state">{{ t("sessionList.searching") }}</div>
        <button
      ...
      <template v-else>
        ...
        <UiEmptyState
          ...
          tone="error"
          :title="t('sessionList.loadFailed')"
          :description="sessionsListError"
          :action-label="t('sessionList.retry')"
        ...
        <template v-else>
          ...
          <template v-if="workspaceGroups.length">
            <div
              ...
              :class="{ 'workspace-group--example': isExampleProject(group.workspace.id) }"
            ...
            >
              <div
                ...
                :class="{
                  ...
                  'list-section-header--example': isExampleProject(group.workspace.id),
                }"
              ...
              >
                ...
                >
                  ...
                  <div v-if="!group.sessions.length" class="chat-list-project-empty">
                    <MessageSquareReply class="chat-list-project-empty__icon" aria-hidden="true" />
...
const workspaceGroups = computed(() => {
  ...
  return groups
    ...
    .sort((left, right) => {
      const leftExample = isExampleProject(left.workspace.id);
      const rightExample = isExampleProject(right.workspace.id);
      return leftExample === rightExample ? 0 : leftExample ? -1 : 1;
    });
});
...
.chat-list-settings-popover__action:hover {
  ...
  background: color-mix(in srgb, var(--app-accent) 12%, var(--app-popup-bg));
}

.workspace-group--example .list-section-title {
  color: #f59e0b;
}
</style>
