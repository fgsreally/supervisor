...
describe("wecode: git-worktree helpers", () => {
  it("sessionBranchName uses short session id", () => {
    expect(sessionBranchName("abcdef12-0000-0000-0000-000000000000")).toBe("wecode/session-abcdef12");
  });

  it("sessionWorktreePath is under .wecode/worktrees", () => {
    const path = sessionWorktreePath("/repo", "sess-1").replace(/\\/g, "/");
    expect(path).toBe("/repo/.wecode/worktrees/sess-1");
  });
  ...
  it("resolveSessionGitContext uses cwd under project worktree", () => {
    ...
    expect(ctx?.branch).toBe("wecode/session-42");
  ...
  it("resolveSessionGitContext returns null without project cwd", () => {
    expect(
      resolveSessionGitContext({
        sessionId: 1,
        cwd: "/repo/.wecode/worktrees/1",
      }),
    ).toBeNull();
  });
});

describe("wecode: session-git-hooks", () => {
