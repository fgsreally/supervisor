...
import { scanWecodeQrCode } from "@/composables/use-native-qr";
import {
  displayNameForUrl,
  getActiveInstanceId,
  listWecodeInstances,
  parseWecodeQrPayload,
  removeWecodeInstance,
  setActiveWecodeInstance,
  type WecodeInstance,
...
const instances = ref<WecodeInstance[]>(listWecodeInstances());
...
function refresh() {
  instances.value = listWecodeInstances();
...
function avatarLetter(item: WecodeInstance): string {
...
async function onScan() {
  scanning.value = true;
  try {
    const raw = await scanWecodeQrCode();
    if (!raw) return;
    const url = parseWecodeQrPayload(raw);
    if (!url) {
...
function onInstanceClick(item: WecodeInstance) {
  if (isMobile.value) {
...
async function connect(id: string) {
  ...
  try {
    const instance = setActiveWecodeInstance(id);
    if (!instance) {
...
async function onDelete(item: WecodeInstance) {
  const label = item.name || displayNameForUrl(item.url);
...
function startLongPress(item: WecodeInstance) {
  clearLongPress();
  longPressFired = false;
