...
import type { SessionWorkflowState, WorkflowStatePatch } from "../src/plugin/types.js";
...
export interface RuntimeOptions {
  ...
  deps: {
    ...
    emitPluginEvent: (event: PluginEvent) => void | Promise<void>;
    exec: (
      command: string,
      args: string[],
...
export function createPluginTestContext(options: RuntimeOptions): Context {
  const services = new SessionPluginServices({
  ...
  let host: TestPluginHost | undefined;
  ...
  const requirePlugin = () => {
    if (!activePluginId) throw new Error("No plugin is active");
  ...
  const context = {
    session: {
      ...
      on<T extends PluginEvent>(
        ...
        requireHost().on(requirePlugin(), event, handler);
      },
    },
    policies: {
      disable: () => {},
    ...
    agent: {
      ...
      registerTool: <TParams extends TSchema, TResult>(
        definition: ToolDefinition<TParams, TResult>,
      ) => requireHost().registerTool(requirePlugin(), definition),
      unregisterTool: (name: string) => requireHost().unregisterTool(requirePlugin(), name),
      activate: async (names: string[]) => {
        requireHost().setToolsActive?.(names, true);
        services.tools.activate(names);
        await options.deps.syncActiveTools();
      ...
      registerCommand: (name: string, definition: PluginCommandDefinition) =>
        requireHost().registerCommand(requirePlugin(), name, definition),
      unregisterCommand: (name: string) =>
        requireHost().unregisterCommand(requirePlugin(), name),
      registerSlash: (name: string, definition: PluginCommandDefinition) =>
        requireHost().registerCommand(requirePlugin(), name, definition),
      unregisterSlash: (name: string) => requireHost().unregisterCommand(requirePlugin(), name),
    ...
    services,
    attachPluginHost(nextHost: TestPluginHost) {
      host = nextHost;
    },
    ...
    removePluginResources(pluginId: string) {
      requireHost().removeResources(pluginId);
    },
    on<T extends PluginEvent>(
      event: T["type"],
      handler: (event: T, eventContext: EventHandlerContext) => void | Promise<void>,
    ) {
      return requireHost().on(requirePlugin(), event, handler);
    },
