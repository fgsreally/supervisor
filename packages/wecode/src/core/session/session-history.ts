...
export function normalizeSessionBranchType(value: string | null): SessionBranchType | null {
  ...
  return value === "subagent" || value === "fork" || value === "btw" ? value : null;
}
...
export async function createSessionCheckpoint(
  db: Pick<WecodeDb, "get" | "updateMeta">,
  sessionId: number,
  ...
  const storage = new SQLiteSessionStorage(db as WecodeDb, sessionId);
