...
import { createWecodeCli } from "./cli-definition.js";
...
import { readWecodeSettings, writeWecodeSettings } from "./utils/wecode-settings.js";
...
import { getWecodeHome, setWecodeHome } from "./utils/wecode-home.js";
...
function _parsePluginFlags(argv: string[]): Record<string, string | boolean | undefined> {
...
if (typeof values.locale === "string") process.env.WECODE_LOCALE = values.locale;
...
async function selectProviderLocal(db: WecodeDb): Promise<Provider> {
  const providers = db.listProviders().filter((p) => p.isEnabled);
  if (providers.length === 0) {
    throw new Error("No providers configured. Run `wecode providers add` first.");
  }
...
function getModelsByProvider(db: WecodeDb, providerId: number) {
  return db.listModels().filter((m) => m.providerId === providerId);
}

function getPrimaryModelId(db: WecodeDb, providerId: number): string | null {
...
async function run() {
  ...
  if (homeArg) setWecodeHome(resolveWorkspacePath(homeArg));
  ...
  const db = new WecodeDb(resolveDbPath());
  ...
  switch (command) {
    case "serve": {
      ...
      process.env.WECODE_URL = `${protocol}://127.0.0.1:${port}`;
      ...
      const devMode = process.env.WECODE_DEV === "1";
      const buildPreviewMode = process.env.WECODE_BUILD_PREVIEW === "1";
      const uiPort = Number(process.env.WECODE_UI_PORT || "5163");
      const publicUrl = devMode ? buildDevPublicUrl(uiPort, Boolean(tls)) : null;
    ...
    case "plugins": {
      ...
      if (subCmd === "install") {
        ...
        if (!source) {
          writeLog("error", "cli.plugins.install.usage");
          process.exit(1);
        }
        const result = await manager.resources.installResource({ kind: "plugin", source });
        const details = result.details ?? {};
        writeLog("info", "cli.plugins.installed", { slug: result.resource.slug });
        writeLog("info", "cli.plugins.rootDir", { value: String(details.rootDir ?? "") });
        writeLog("info", "cli.plugins.entry", { value: String(details.entryPath ?? "") });
        writeLog("info", "cli.plugins.deps", {
      ...
      if (subCmd === "update") {
        const id = cmdArgs[1];
        if (!id) {
          writeLog("error", "cli.plugins.update.usage");
          process.exit(1);
        }
        const result = await manager.resources.updateResource("plugin", id);
        const details = result.details ?? {};
        writeLog("info", "cli.plugins.updated", { slug: result.resource.slug });
        writeLog("info", "cli.plugins.rootDir", { value: String(details.rootDir ?? "") });
        writeLog("info", "cli.plugins.entry", { value: String(details.entryPath ?? "") });
        writeLog("info", "cli.plugins.deps", {
          value: String(details.installCommand ?? "none"),
      ...
      if (subCmd === "uninstall") {
        const id = cmdArgs[1];
        if (!id) {
          writeLog("error", "cli.plugins.uninstall.usage");
          process.exit(1);
        }
        await manager.resources.uninstallResource("plugin", id);
        writeLog("info", "cli.plugins.uninstalled", { id });
        db.close();
        break;
      }

      if (subCmd === "list") {
        const resources = manager.resources.listResources("plugin");
        if (resources.length === 0) {
          writeLog("info", "cli.plugins.none");
        } else {
          for (const resource of resources) {
            const ver = resource.version ? ` v${resource.version}` : "";
            writeLog("info", "cli.plugins.item", {
              slug: resource.slug,
      ...
      if (subCmd === "bind") {
        ...
        const id = cmdArgs[2];
        if (!agentIdRaw || !id) {
          writeLog("error", "cli.plugins.bind.usage");
          process.exit(1);
        }
        const agentId = Number(agentIdRaw);
        if (!Number.isFinite(agentId)) {
          writeLog("error", "cli.plugins.agentIdNumber");
          process.exit(1);
        }
        manager.resources.bindResource({ agentId, kind: "plugin", slug: id });
        writeLog("info", "cli.plugins.bound", { id, agentId });
        db.close();
        break;
      }
      ...
      if (subCmd === "unbind") {
        ...
        const id = cmdArgs[2];
        if (!agentIdRaw || !id) {
          writeLog("error", "cli.plugins.unbind.usage");
          process.exit(1);
        }
        const agentId = Number(agentIdRaw);
        if (!Number.isFinite(agentId)) {
        ...
        await manager.resources.unbindResource({ agentId, kind: "plugin", slug: id });
        writeLog("info", "cli.plugins.unbound", { id, agentId });
        db.close();
        break;
      }

      writeLog("error", "cli.plugins.unknown", { command: subCmd ?? "" });
      showHelp();
      db.close();
      process.exit(1);
      break;
    ...
    case "config": {
      ...
      const ensureApiKey = async (provider: CredentialProvider): Promise<void> => {
        const current = readWecodeSettings();
        const envFields = {
        ...
        writeWecodeSettings({ [encryptedFields[provider]]: encryptApiKey(apiKey) });
      ...
      if (section === "show") {
        ...
        writeWecodeSettings({ webFetchProvider: provider as (typeof fetchProviders)[number] });
        if (provider !== "native") {
        ...
        writeWecodeSettings({ browserMode: mode as (typeof browserModes)[number] });
