 ...
 * It just verifies the file system operations: install → discover → link → uninstall.
 */
...
describe("extension installation and discovery", () => {
	...
	it("installs a local extension into global catalog and discovers it", async () => {
		const { installExtensionToGlobal, uninstallGlobalExtension } = await import(
			`${MODULE_BASE}/agent/extension-installer.js`
		);
		const { listExtensionInfosInDirectories } = await import(`${MODULE_BASE}/extension-system/loader.js`);
		const { getGlobalResourceDirs } = await import(`${MODULE_BASE}/agent/agent-paths.js`);
	...
	it("discovers extensions linked to an agent", async () => {
		const { installExtensionToGlobal, uninstallGlobalExtension } = await import(
			`${MODULE_BASE}/agent/extension-installer.js`
		);
		const { listExtensionInfosInDirectories } = await import(`${MODULE_BASE}/extension-system/loader.js`);
		const { getGlobalResourceDirs, getAgentHomeDir, linkGlobalResourceToAgent } = await import(
			`${MODULE_BASE}/agent/agent-paths.js`
		);
		...
		writeFileSync(
			join(srcRoot, "index.ts"),
			`import { defineExtension } from "@earendil-works/pi-supervisor";\nexport default defineExtension({ name: "link-test", setup() {} });\n`,
			"utf8",
		);
		...
		const agentHome = getAgentHomeDir(agentId);
		const linkPath = linkGlobalResourceToAgent(agentHome, "extensions", result.rootDir);
		expect(existsSync(linkPath)).toBe(true);

		const agentExtDir = join(agentHome, "extensions");
		const agentInfos = listExtensionInfosInDirectories([agentExtDir]);
		expect(agentInfos.some((i) => i.id === result.id)).toBe(true);
	...
	it("update requires package.json repository field", async () => {
		const { installExtensionToGlobal, uninstallGlobalExtension, updateGlobalExtension } = await import(
			`${MODULE_BASE}/agent/extension-installer.js`
		);
