...
function assertAgentExists(db: WecodeDb, agentId: number): void {
...
export class ResourceManager {
  ...
  bindResource(input: BindResourceInput): AgentResourceBinding {
    ...
    if (resource.meta?.builtin === true && resource.kind === "plugin") {
  ...
  async unbindResource(input: UnbindResourceInput): Promise<void> {
    const resource =
      "kind" in input
        ...
        : this.deps.db.getResource(input.resourceId);
    if (resource?.meta?.builtin === true) {
      throw new Error(`Cannot remove built-in plugin "${resource.slug}" from an agent`);
    }
    if ("kind" in input) {
