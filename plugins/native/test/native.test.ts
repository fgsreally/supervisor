...
const pluginEntry = resolve(dirname(fileURLToPath(import.meta.url)), "..", "index.ts");
...
describe("wecode-native", () => {
  it("loads plugin entry", async () => {
    const mod = await import(pluginEntry);
    expect(mod.default.name).toBe("wecode-native");
  ...
  it("executes bash through pi-natives when platform addon is available", async () => {
    ...
    const result = await tool.execute(
      "test-call",
      { command: "echo wecode-native", intent: "verify rust shell bridge" },
      undefined,
      undefined,
    );

    expect(textContent(result)).toContain("wecode-native");
  ...
  it("greps workspace via native grep", async () => {
    ...
    const result = await tool.execute(
      "test-call",
      { pattern: "wecode-native", path: ".", literal: true },
      undefined,
      undefined,
    );

    expect(textContent(result)).toMatch(/wecode-native/);
    expect((result.details as { engine?: string }).engine).toBe("pi-natives");
  }, 30_000);
