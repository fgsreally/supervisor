...
export function buildSkillFileTree(files: UISkillFile[]): SkillTreeNode[] {
...
function sortSkillTree(nodes: SkillTreeNode[]): SkillTreeNode[] {
	const folders = nodes.filter((n) => n.children?.length).sort((a, b) => a.text.localeCompare(b.text));
	const files = nodes.filter((n) => !n.children?.length).sort((a, b) => a.text.localeCompare(b.text));
