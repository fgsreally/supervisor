...
Wecode 核心只保存一个轻量阶段标签：`sessions.stage`。实现位于
`src/core/session-workflow.ts`，插件可在其上实现自己的阶段机。
...
`workflow` 是兼容命名，`status` 恒为 `working`，不会持久化。插件若需要
`waiting_confirmation`、`waiting_choice`、`completed` 等状态，必须写自己的 namespaced
`sessions.meta`，不能再写旧的 `meta.workflow`。

插件 API 对应 `ctx.session.workflow.get/set/clear`，阶段变化触发
`workflow.stage_changed`。`workflow.status_changed` 只保留在兼容类型中，新插件不应依赖它。

## Strict SDD

`plugins/strict-sdd` 为主 Session 提供阶段式流程：

`Brainstorm → Design → Spec → Mockup → Planning → Test/Vertical → Implement/Verify → Archive`

当前阶段使用 `sessions.stage`；插件状态使用 `sessions.meta.strictSdd.status`；change、循环和
