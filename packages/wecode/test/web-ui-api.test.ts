...
beforeEach(() => {
  tmpDir = join(tmpdir(), `wecode-web-ui-test-${Date.now()}`);
...
describe("wecode: Web UI API compatibility", () => {
  ...
  describe("Agent management", () => {
    ...
    it("POST /agents creates an isolated ACP agent without a provider", async () => {
      ...
      expect(resources.layers.agent).toEqual({ skills: [], prompts: [], plugins: [], mcp: [] });
