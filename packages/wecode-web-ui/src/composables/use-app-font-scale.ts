...
export const APP_FONT_SCALE_STORAGE_KEY = "wecode-ui-font-scale-v2";
const LEGACY_FONT_SCALE_STORAGE_KEY = "wecode-ui-font-scale";
...
export function initAppFontScale(): void {
  ...
  if (typeof localStorage !== "undefined") {
    ...
    if (stored !== null) {
      ...
      const migrated = migrateLegacyFontScale(
        localStorage.getItem(LEGACY_FONT_SCALE_STORAGE_KEY) ??
          localStorage.getItem("pi-supervisor-ui-font-scale"),
      );
      if (migrated) initial = migrated;
      else {
        const oldV2 = localStorage.getItem("pi-supervisor-ui-font-scale-v2");
        if (oldV2 !== null) initial = parseAppFontScale(oldV2);
      }
    }
  }
