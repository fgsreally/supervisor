const SERVER_URL_KEY = "wecode.serverUrl";
const SERVER_PIN_KEY = "wecode.serverPin";
const DEVICE_ID_KEY = "wecode.deviceId";
const BACKGROUND_CONNECTION_KEY = "wecode.backgroundConnection";
const INSTANCES_KEY = "wecode.instances";
const ACTIVE_INSTANCE_KEY = "wecode.activeInstanceId";

export interface WecodeInstance {
  id: string;
...
export function listWecodeInstances(): WecodeInstance[] {
  migrateLegacyInstance();
  try {
    const raw = localStorage.getItem(INSTANCES_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as WecodeInstance[];
...
function writeInstances(instances: WecodeInstance[]): void {
...
export function getActiveWecodeInstance(): WecodeInstance | null {
  const id = getActiveInstanceId();
  if (!id) return null;
  return listWecodeInstances().find((item) => item.id === id) ?? null;
}

/** Sync legacy single-key fields used by api.ts / bootstrap. */
function syncActiveKeys(instance: WecodeInstance | null): void {
  if (!instance) {
...
export function setActiveWecodeInstance(id: string): WecodeInstance | null {
  const instances = listWecodeInstances();
  const index = instances.findIndex((item) => item.id === id);
  if (index < 0) return null;
  const updated: WecodeInstance = { ...instances[index]!, lastUsedAt: Date.now() };
...
export function upsertWecodeInstance(input: {
  id?: string;
  name?: string;
  url: string;
  pin: string;
...
export function removeWecodeInstance(id: string): void {
  const next = listWecodeInstances().filter((item) => item.id !== id);
  writeInstances(next);
...
export function clearActiveWecodeInstance(): void {
  localStorage.removeItem(ACTIVE_INSTANCE_KEY);
  localStorage.removeItem(SERVER_URL_KEY);
  localStorage.removeItem(SERVER_PIN_KEY);
}

export function hasConfiguredWecodeInstance(): boolean {
  const active = getActiveWecodeInstance();
...
export function parseWecodeQrPayload(raw: string): string | null {
