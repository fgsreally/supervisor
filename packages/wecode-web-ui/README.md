...
Vue 3 + Vite 前端，连接 `wecode` HTTP API，提供聊天、Agent 管理、全局资源库等功能。
...
- 后端：`packages/wecode/src/tools/ask/`，工具名 `ask`
...
- Agent 名称：**WeCode 助手**（`meta.builtin` + `meta.userSpawnable`；数据库自增 id，非固定字符串）
- 置顶会话：`meta.pinned` + `meta.builtin`
- 启动 wecode 时自动创建；自带 `wecode-guide` skill 与系统提示
- 用途：Wecode 配置、插件迁移、skill 安装、使用答疑
- 会话列表未读角标：`meta.unread`（打开会话时清零）

## 浏览器推送

- 消息流完成且页面在后台时，通过浏览器 `Notification` API 推送（需用户授权）
- 逻辑：`src/composables/use-notifications.ts`
- 静音会话（`meta.muted`）不推送
- 原生 App 走 Capacitor 本地通知，不经过 Service Worker

## 全局资源

- 默认全局根：`~/.wecode`；开发时 `--cwd playground`
- 其下：`global/{skills,plugins,prompts}`、`agents/`、`public/`、`wecode.db`
