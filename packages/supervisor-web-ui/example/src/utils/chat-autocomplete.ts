import { getLinkedResourcesForAgent } from '../mock/store'
import { getWorkspaceFiles } from '../mock/workspace-files'
import { isTokenStart } from './chat-token-patterns'
import { getFileIconKind, type FileIconKind } from './file-type-icon'
...
function extractAtPrefix(textBeforeCursor: string): string | null {
	...
	if (quoteStart !== null) {
		if (quoteStart > 0 && textBeforeCursor[quoteStart - 1] === '@' && isTokenStart(textBeforeCursor, quoteStart - 1)) {
...
export function getAutocompleteContext(text: string, cursor: number): ChatAutocompleteContext | null {
...
export function getAutocompleteSuggestions(
	...
	if (context.trigger === 'at') {
		...
		const files = getWorkspaceFiles(options.workspaceId)
		const filtered = fuzzyFilter(files, rawQuery, (f) => f.path)
	...
	const skills =
		options.agentId != null
			? getLinkedResourcesForAgent(options.agentId).filter((r) => r.kind === 'skills')
			: []

	const skillItems: ChatAutocompleteItem[] = skills.map((skill) => {
	...
	const filtered = fuzzyFilter(skillItems, query, (item) => item.value)
