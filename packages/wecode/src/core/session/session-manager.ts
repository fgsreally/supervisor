...
import { PluginModuleRegistry } from "../../plugin/registry.js";
import { ResourceManager } from "../../resources/resource-manager.js";
import {
  ensureAgentBuiltinPluginBindings,
  ensureBuiltinPluginResources,
  listEnabledBuiltinPluginSlugs,
} from "../../plugin/builtin/ensure.js";
import {
  BUILTIN_PLUGINS,
  BUILTIN_PLUGIN_SLUGS,
  isBuiltinPluginResource,
} from "../../plugin/builtin/catalog.js";
import { declaredMenusForSlug, serializeUiMenu } from "../../plugin/ui-menus.js";
import { JobManager } from "../jobs/jobs.js";
import {
...
import {
  disposeAgentPluginRuntime,
  loadSessionPlugins,
  PluginAttachRuntime,
} from "./session-plugin-attach.js";
import {
...
import { attachHomeTaskSessionSync } from "../tasks/home-task-sync.js";
...
import { isFeatureModelRef, readWecodeSettings } from "../../utils/wecode-settings.js";
...
import { listPluginInfosInDirectories } from "../../plugin/index.js";
...
import {
  ensureSessionDir,
  getSessionDir,
  removeProjectDirSync,
  removeSessionDirSync,
} from "./session-files.js";
import { removeSessionMediaDirSync, type SessionPromptImage } from "./session-media.js";
import {
  buildSessionUserPrompt,
  type SessionPastedTextInput,
  type SessionPromptAttachment,
} from "./session-pasted-text.js";
import { runShadow } from "../../plugin/builtin/shadow/index.js";
import {
...
import { getGlobalPluginsDirectory } from "../../plugin/resource.js";
...
export class SessionManager {
  ...
  private readonly pluginRegistry = new PluginModuleRegistry();
  ...
  listAgentUiMenus(agentId: number): UiMenuDescriptor[] {
    ...
    for (const slug of listEnabledBuiltinPluginSlugs(this.db, agentId)) {
      for (const menu of declaredMenusForSlug(slug)) byId.set(menu.id, menu);
    }
    const userSlugs = this.db
      .listAgentResourceBindings(agentId, { kind: "plugin", enabledOnly: true })
      .flatMap((binding) => {
        const slug = binding.resource?.slug;
        if (!slug || BUILTIN_PLUGIN_SLUGS.has(slug)) return [];
        return [slug];
      });
    for (const module of this.pluginRegistry.getMany(userSlugs)) {
  ...
  constructor(db: WecodeDb) {
    ...
    for (const input of this.db.listPersistedSessionInputs()) {
      this.sessionInputQueues.enqueue(input.sessionId, {
        ...
        pastedTexts: input.pastedTexts as SessionPastedTextInput[] | undefined,
        attachments: input.attachments as SessionPromptAttachment[] | undefined,
      });
    }
    ...
    this.resourceHandlers = createResourceHandlers({
      ...
      deactivateAgentPlugin: (agentId, slug) => this.deactivateAgentPlugin(agentId, slug),
    });
  ...
  async ensureResourceCatalog(): Promise<void> {
    ...
    ensureBuiltinPluginResources(this.db);
    await this.pluginRegistry.refresh(this.db);
    this.resourcesInitialized = true;
  }

  getPluginRegistry(): PluginModuleRegistry {
    return this.pluginRegistry;
  }

  /** Unified resource install / bind API (CLI, HTTP, plugins). */
  ...
  private _listSessions(filter?: Parameters<WecodeDb["list"]>[0]): Session[] {
  ...
  private assembleSessionTools(
    ...
    const baseTools =
      ...
      createDefaultTools(cwd, toolsPreset, {
        sessionId,
        projectId: this.db.get(sessionId)?.project_id,
  ...
  private getSessionBashEnv(sessionId: number): NodeJS.ProcessEnv {
    ...
    const env: NodeJS.ProcessEnv =
      row.project_id == null ? {} : { SV_SESSION_DIR: getSessionDir(row.project_id, sessionId) };
    try {
      const meta = JSON.parse(row.meta || "{}") as Record<string, unknown>;
      return { ...sessionServicePortEnv(meta), ...env };
    } catch {
      return env;
    }
  }
  ...
  async resolveAgentResources(agentId: number, cwd: string) {
    ...
    if (agent?.backendType !== "native") {
      return {
        ...
        tools: [],
        layers: {
          agent: { skills: [], prompts: [], plugins: [], mcp: [] },
          project: { skills: [], prompts: [], plugins: [], mcp: [] },
        },
      };
    }
    await this.ensureResourceCatalog();
    return resolveAgentResources(this.db, agentId, cwd, this.pluginRegistry);
  }
  ...
  async resolveAgentTools(agentId: number, cwd: string) {
    ...
    await this.ensureResourceCatalog();
    return resolveAgentTools(this.db, agentId, cwd, this.pluginRegistry);
  }
  ...
  private async attachExternalSessionPlugins(
    session: Session,
    agent: Agent,
    startRuntime: (session: Session) => Promise<ManagedSessionRuntime>,
    setupReason: import("../../plugin/types.js").SessionSetupReason,
    ...
    const bridge = new PluginAttachRuntime(session.id);
    const host = await loadSessionPlugins({
      runtime: bridge,
      agentId: session.agentId ?? 0,
  ...
  private async restoreRuntime(id: number): Promise<ManagedSessionRuntime> {
    ...
    try {
      ...
      if (agent && agent.backendType !== "native") {
        const runtime = await timedSessionStep(id, "restoreRuntime/createExternalRuntime", () =>
          session.externalSessionId
            ...
            : this.attachExternalSessionPlugins(
                session,
                agent,
      ...
      const refreshedAfterPlugins = this.get(session.id)!;
      runtime.syncWorkingDirectory(refreshedAfterPlugins.cwd);
      if (agent?.backendType === "native") {
        runtime.configureAgentPermissions(
          agent.permissionRules,
          refreshedAfterPlugins.cwd,
          (request) => this.requestSessionApproval(session.id, request),
        );
      }
      const pluginTools = runtime.collectPluginTools();
      if (pluginTools.length > 0) {
        const mergedTools = new Map<string, AgentTool>();
        for (const tool of tools) mergedTools.set(tool.name, tool);
        for (const tool of pluginTools) mergedTools.set(tool.name, tool);
  ...
  create(options: CreateSessionOptions = {}): Session {
    ...
    // Prefer explicit cwd; otherwise seed from the bound project so plugins
    // (worktree, file tree) never fall back to the wecode playground root.
   ...
   * Resources (skills/prompts/plugins) follow session.agentId — main or child session.
   *
  ...
  private async finalizeSpawn(
    ...
    try {
      ...
      const refreshedAfterPlugins = this.get(activeSession.id)!;
      runtime.syncWorkingDirectory(refreshedAfterPlugins.cwd);
  ...
  upsertPushDevice(input: import("../push/push-device-types.js").PushDeviceInput) {
  ...
  /** Rebuild plugin modules and tool wiring for active sessions of an agent. */
  async reloadNativeAgentPlugins(agentId: number): Promise<void> {
    const agent = this.db.getAgent(agentId);
    if (!agent) return;
    await disposeAgentPluginRuntime(this, agentId);
    ...
    for (const [sessionId, runtime] of this.runtimes) {
      ...
      if (runtime instanceof SessionRuntime) {
        await runtime.reloadPlugins(agentId, agent.name, session.cwd, this.db, this);
        await this.refreshRuntimeTools(sessionId, runtime, session, agent);
        continue;
      }

      if (session.projectId == null) continue;
      await runtime.plugin?.clear().catch(() => {});
      const resource = new AgentResource({
        sessionId,
        agentId,
        agent,
  ...
  private async refreshRuntimeTools(
    ...
    const pluginTools = runtime.collectPluginTools();
    for (const tool of pluginTools) merged.set(tool.name, tool);
  ...
  async submitSessionInput(
    ...
    const session = this.get(id);
    if (!session) throw new Error(`Session ${id} not found`);
    const prompt = await buildSessionUserPrompt(
      session.projectId,
      session.id,
      input.message,
      input.pastedTexts,
    ...
    const entry: SessionQueuedInput = {
      ...
      images: input.images,
      origin: input.origin,
      ...(input.pastedTexts?.length || input.attachments?.length
        ? {
            displayMessage: input.message,
            ...(input.pastedTexts?.length ? { pastedTexts: input.pastedTexts } : {}),
          }
        : {}),
  ...
  private isLatestUserMessage(sessionId: number, message: string): boolean {
    let entryId = this.db.get(sessionId)?.leaf_id;
    while (entryId) {
      const row = this.db.getMessageRowByEntryId(sessionId, entryId);
  ...
  private async unloadEval(id: number): Promise<void> {
    ...
    await runtime.plugin.unload("eval").catch(() => undefined);
  }
  ...
  async complete(id: number): Promise<Session> {
    ...
    // Emit session.before_complete (plugins: stop → uninstall → worktree on achieve)
    const runtime = this.runtimes.get(id);
    if (runtime?.plugin) {
      await runtime.plugin.emit({
  ...
  async emitSessionPluginEvent(sessionId: number, event: PluginEvent): Promise<void> {
    ...
    await runtime.plugin?.emit(event);
  }
  ...
  async syncSession(id: number): Promise<Session> {
    ...
    if (!runtime.plugin) throw new Error("Session plugins are not loaded");
  ...
  list(filter?: Parameters<WecodeDb["list"]>[0]): Session[] {
  ...
  async parseProject(projectId: number): Promise<{
    ...
    const ref = readWecodeSettings().featureModels?.assistant;
  ...
  insertAgent(
    row: Parameters<WecodeDb["insertAgent"]>[0],
    ...
    ensureBuiltinPluginResources(this.db);
    ensureAgentBuiltinPluginBindings(this.db, agent.id);
  ...
  /** Unified plugin list for Agent management UI (builtins + user bindings). */
  listAgentPlugins(agentId: number): Array<{
    slug: string;
    name: string;
    description: string | null;
    builtin: boolean;
    ...
    ensureAgentBuiltinPluginBindings(this.db, agentId);
    ...
    for (const binding of bySlug.values()) {
      ...
      if (isBuiltinPluginResource(resource.meta)) continue;
      rows.push({
        slug: resource.slug,
  ...
  setAgentPluginEnabled(agentId: number, resourceId: number, enabled: boolean) {
    const binding = this.resourceManager.setResourceEnabled(agentId, resourceId, enabled);
    this.publishAgentUiMenus(agentId);
    return binding;
  }

  updateAgent(id: number, patch: Parameters<WecodeDb["updateAgent"]>[1]): AgentWithSystemMd {
  ...
  private async emitStageChange(
    ...
    const plugin = this.runtimes.get(id)?.plugin;
    if (!plugin) return;
    await plugin.emit({
  ...
  /** @deprecated thin adapter over getStage; kept for plugin API compatibility. */
  getWorkflow(id: number): { stage: string; status: "working" } | null {
    const stage = this.getStage(id);
    return stage ? { stage, status: "working" } : null;
  }

  /** @deprecated thin adapter over setStage; kept for plugin API compatibility. */
  async setWorkflow(
    id: number,
  ...
  /** @deprecated thin adapter over clearStage; kept for plugin API compatibility. */
   ...
   * Same idea as plugin `ctx.session.sendCustomMessage`.
   */
  ...
  private async recordLlmError(sessionId: number, content: string): Promise<void> {
    ...
    // Keep one actionable error card per Session; older failures are no longer useful
    // once a later turn has failed.
    this.deleteLlmErrorLeaf(sessionId);
    const storage = new SQLiteSessionStorage(this.db, sessionId);
  ...
  private deleteLlmErrorLeaf(sessionId: number): string | null {
    const errors = this.db.getMessageRows(sessionId).filter((row) => {
    ...
    this.db.db.transaction(() => {
      for (const error of errors) {
        this.db.db
          .prepare(
            "UPDATE messages SET parent_entry_id = ? WHERE session_id = ? AND parent_entry_id = ?",
          )
          .run(error.parent_entry_id, sessionId, error.entry_id);
        this.db.db
          .prepare(
            "UPDATE sessions SET leaf_id = ?, last_active_at = ? WHERE id = ? AND leaf_id = ?",
          )
          .run(error.parent_entry_id, Date.now(), sessionId, error.entry_id);
        this.db.db
          .prepare("DELETE FROM messages WHERE session_id = ? AND entry_id = ?")
          .run(sessionId, error.entry_id);
      }
    })();
    return errors[0]?.entry_id ?? null;
  }
  ...
  async retryAfterLlmError(id: number): Promise<Session> {
    const row = this.db.get(id);
    if (!row) throw new Error(`Session ${id} not found`);
    if (row.status !== "error" && row.status !== "active" && row.status !== "idle") {
      throw new Error(`Session ${id} cannot be retried while ${row.status}`);
    }

    // Restore first. If Codex/another external process still owns the thread,
    // keep the error card and status so closing it makes retry possible.
    const runtime = await this.getOrRestoreRuntime(id);
    ...
    if (runtime instanceof SessionRuntime) {
      ...
      const content =
        lastUser?.type === "message"
          ? (lastUser.message as { content?: unknown }).content
          : undefined;
      const text = Array.isArray(content)
        ? content
            ...
            .join("\n")
        : "";
      if (!text.trim()) throw new Error("No user message available to retry");
  ...
  updateModel(providerId: number, modelId: string, patch: UpdateModelOptions) {
    const dbPatch: Parameters<WecodeDb["updateModel"]>[2] = {};
  ...
  updateProvider(id: number, patch: Parameters<WecodeDb["updateProvider"]>[1]) {
  ...
  /**
   * Resolve global resource catalog from Wecode global/ plus npx skills global dirs.
  ...
  resolveGlobalResources(): ResourceLayer {
    ...
    const pluginInfos = listPluginInfosInDirectories([globalPluginDir]);

    return {
      skills: skillsToResourceInfo(skills),
      prompts: promptsToResourceInfo(promptTemplates),
      plugins: pluginInfos.map((info) => ({
        id: info.id,
  ...
  private async deactivateAgentPlugin(agentId: number, slug: string): Promise<void> {
    const pluginId = this.pluginRegistry.get(slug)?.definition.name ?? slug;
    const activeRuntimes = [...this.runtimes.entries()]
      .filter(([sessionId]) => this._getSession(sessionId)?.agentId === agentId)
      .map(([, runtime]) => runtime);
    await Promise.all(activeRuntimes.map((runtime) => runtime.deactivatePlugin(pluginId)));
  }
  ...
  private resolveDefaultSpawnAgentId(): number {
    ...
    const preferred =
      agents.find((agent) => agent.name === "WeCode 助手") ??
