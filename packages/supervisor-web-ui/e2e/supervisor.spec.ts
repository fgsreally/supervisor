...
test.describe('Supervisor Web UI E2E', () => {
  ...
  test.describe('Session Management', () => {
    ...
    test('should create a new session', async ({ page }) => {
      ...
      if (await newSessionBtn.count() > 0) {
        ...
        if (await nameInput.count() > 0) {
          ...
          // Submit
          const submitBtn = page.locator('button[type="submit"], button:has-text("创建"), button:has-text("Create")')
    ...
    test('should navigate to session detail', async ({ page }) => {
      ...
      if (await firstSession.count() > 0) {
        ...
        await expect(page.locator('[data-testid="chat-view"], .chat-view, [class*="chat"]')).toBeVisible()
      }
    })
  })
  ...
  test.describe('Agent Management', () => {
    test('should display agent list', async ({ page }) => {
      ...
      const contactsLink = page.locator('a:has-text("Contacts"), a:has-text("联系人"), [data-testid="contacts-link"]')
  ...
  test.describe('Provider Management', () => {
    test('should display provider list', async ({ page }) => {
      ...
      const settingsLink = page.locator('a:has-text("Settings"), a:has-text("设置"), [data-testid="settings-link"]')
  ...
  test.describe('Resource Management', () => {
    test('should display resources', async ({ page }) => {
      ...
      const resourcesLink = page.locator('a:has-text("Resources"), a:has-text("资源"), [data-testid="resources-link"]')
  ...
  test.describe('API Integration', () => {
    test('should handle API errors gracefully', async ({ page }) => {
      ...
      const errorMessage = page.locator('[data-testid="error-message"], [class*="error"], .error-message')
