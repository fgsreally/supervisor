...
export class AcpSessionRuntime implements ManagedSessionRuntime {
  ...
  private sessionProjectId(): number {
  ...
  static async create(options: {
    ...
    if (options.session.projectId == null) throw new Error("session has no project");
    const env = {
      ...process.env,
      ...config.env,
      ...portEnv,
      SV_SESSION_DIR: getSessionDir(options.session.projectId, options.session.id),
    };
    ...
    try {
      await Promise.race([
        connection.initialize({
          protocolVersion: 1,
          clientCapabilities: {},
          clientInfo: { name: "wecode", version: "0.1.0" },
        }),
  ...
  private async runPrompt(
    ...
    const imageContent = images?.length
      ? await resolveSessionPromptImages(this.sessionProjectId(), this.id, images)
      : undefined;
