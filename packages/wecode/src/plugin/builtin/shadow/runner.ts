...
export async function runShadow(
  ...
  try {
    await manager.emitSessionPluginEvent(session.id, startEvent);
  } catch (error: unknown) {
  ...
  const pluginKeys = Object.keys(pluginProperties);

  let resultSchema: TSchema;
  try {
    resultSchema = createShadowResultSchema(pluginProperties);
  } catch (error: unknown) {
    const detail = error instanceof Error ? error.message : String(error);
  ...
  try {
    ...
    const normalized = normalizeShadowSubmitResult(run.result, resultSchema, pluginKeys);
    if (!normalized) {
  ...
  void manager
    .emitSessionPluginEvent(session.id, {
