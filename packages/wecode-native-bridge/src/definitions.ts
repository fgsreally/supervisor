...
export interface WecodeNativePlugin {
  /** Register for remote push (token forwarded to Wecode server by the app). */
  getPushToken(): Promise<{ token: string | null; platform: "ios" | "android" | "web" }>;

  /** Native share / Open In: files received before web-ui consumes them. */
