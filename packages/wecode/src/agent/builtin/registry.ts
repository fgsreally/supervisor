...
import { ensurePluginExternalAgents } from "../../plugin/external-agents.js";
...
> = {
  intro: {
    name: "Intro",
    description: "Wecode guide and plugin authoring assistant",
    toolsPreset: "coding",
  },
  coding: {
    name: "Coding",
...
function pickProvider(db: WecodeDb): { modelId: number } | null {
...
function ensurePackagedAgent(db: WecodeDb, kind: PackagedAgentKind): number | undefined {
  ...
  if (existing !== undefined) {
    ...
    ensureAgentBuiltinPluginBindings(db, existing);
    return existing;
  }
...
function removeLegacyUtilityAgents(db: WecodeDb): void {
  const legacyIds = db
    .listAgents()
    .filter(
      (agent) =>
...
export function ensurePackagedAgents(db: WecodeDb): void {
  ensurePluginExternalAgents(db, "builtin:trae", [
    {
      id: "trae",
      name: "Trae",
...
const BUILTIN_ASSISTANT_NAME = "WeCode 助手";
...
function findBuiltinAssistantId(db: WecodeDb): number | undefined {
  return db.listAgents().find((agent) => agent.name === BUILTIN_ASSISTANT_NAME && agent.isBuiltin)
    ?.id;
}

function findBuiltinAssistantSessionId(db: WecodeDb, agentId: number): number | undefined {
...
export function dedupeBuiltinAssistantSessions(db: WecodeDb): void {
...
function installBuiltinAssistantSkill(db: WecodeDb, agentId: number): void {
  const skillDir = join(getGlobalSkillsDirectory(), "wecode-guide");
...
export function ensureBuiltinAssistant(db: WecodeDb, manager: SessionManager): void {
  ...
  if (!agent) {
    agent = db.insertAgent({
      ...
      description: "Wecode 内置助手，用于配置和管理资源",
