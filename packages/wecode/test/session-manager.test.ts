...
beforeEach(() => {
  ...
  tmpDir = join(tmpdir(), `wecode-im-test-${Date.now()}`);
...
describe("wecode: SessionManager", () => {
  ...
  it("persists stage and emits stage changes", async () => {
    ...
    manager.getRuntime(inst.id).plugin!.on("workflow.stage_changed", (event) => {
  ...
  it("does not delete builtin assistant sessions directly", async () => {
    ...
    await expect(manager.delete(session.id)).rejects.toThrow("WeCode 助手不能删除");
  ...
  it("restores queued Session input after Wecode restarts", async () => {
    const session = manager.create();
    const busyManager = manager as unknown as {
      isSessionBusy(sessionId: number): Promise<boolean>;
    };
  ...
  it("normalizes interrupted Session states when Wecode restarts", async () => {
