...
export type ResourceItem =
  ...
  | { kind: "plugins"; info: PluginResourceInfo }
...
export const useSessionStore = defineStore("session", () => {
  ...
  const messageCursors = ref<
    Record<string, { oldestRowId: number | null; newestRowId: number | null; hasMore: boolean }>
  >({});
  ...
  function persistProjectSessionCache() {
    void writeClientCache("project-session", {
      projects: projects.value,
      sessions: sessions.value,
    });
  }

  function applyProjectSessionSnapshot(snapshot: api.ProjectSessionSnapshot) {
    projects.value = sortByProjectPreference(snapshot.projects);
  ...
  async function loadProjectSessionSnapshot() {
    return loadClientResource<api.ProjectSessionSnapshot>({
      key: "project-session",
      cacheKey: "project-session",
      groupKey: "project-session",
      read: async () => {
        const [projects, sessions] = await Promise.all([api.listProjects(), api.listSessions()]);
        return { projects, sessions };
      },
      apply: applyProjectSessionSnapshot,
      loading: (value) => {
        root.loading.sessions = value;
      },
    });
  }
  ...
  async function fetchProjects() {
    root.clearError();
    try {
      await loadProjectSessionSnapshot();
      persistSessionListCache();
  ...
  async function deleteProject(id: string, name: string) {
    root.clearError();
    try {
      await api.deleteProject(id, name);
      projects.value = projects.value.filter((project) => project.id !== id);
      persistSessionListCache();
      persistProjectSessionCache();
    } catch (err) {
      root.setError(err instanceof Error ? err.message : "Failed to delete project");
      throw err;
    }
  }
  ...
  async function fetchSessions(params?: {
    ...
    try {
      if (hasQuery) {
        // Filtered lists are not part of the startup cache group.
        const value = await api.listSessions(query);
  ...
  async function fetchSession(id: string) {
    root.clearError();
    try {
      // Status/preview changes arrive outside the cached project snapshot.
  ...
  async function fetchSessionMessages(id: string, fresh = false) {
    root.clearError();
    try {
      ...
      await syncClientResource<api.SessionMessagesPage>(
        fresh ? { ...resource, force: true } : resource,
      );
  ...
  async function sendPrompt(id: string, message: string) {
    ...
    return api.promptSession(
      ...
      () => {
        ...
        void fetchSessionMessages(id, true);
      },
    );
  }
  ...
  function setSessionStatus(id: string, status: api.SessionStatus) {
...
export const useAgentStore = defineStore("agent", () => {
  ...
  function persistAgentsCache() {
    void writeClientCache("agents", agents.value);
  }
    ...
    const groups: Array<{ label: string; agents: typeof agents.value }> = [
      {
        ...
        agents: agents.value.filter(
          (a) =>
            ...
            (a.name === "WeCode 助手" || a.name === "Coding"),
        ),
      },
      {
  ...
  async function fetchAgentResources(id: string, cwd?: string) {
    ...
    try {
      return await loadClientResource<api.AgentResources>({
        ...
        apply: (value) => {
          agentResources.value[id] = value;
        },
        loading: (value) => {
          root.loading.resources = value;
        },
      });
    } catch (err) {
  ...
  async function fetchAgentPlugins(id: string) {
    root.clearError();
    try {
      return await api.listAgentPlugins(id);
    } catch (err) {
      root.setError(err instanceof Error ? err.message : "Failed to fetch plugins");
      throw err;
    }
  }

  async function setAgentPluginEnabled(id: string, resourceId: number, enabled: boolean) {
    root.clearError();
    try {
      return await api.setAgentPluginEnabled(id, resourceId, enabled);
    } catch (err) {
      root.setError(err instanceof Error ? err.message : "Failed to update plugin");
      throw err;
    }
  }
...
export const useProviderStore = defineStore("provider", () => {
  ...
  function persistProviderModelCache() {
    void writeClientCache("provider-model", {
  ...
  function applyProviderModelSnapshot(snapshot: api.ProviderModelSnapshot) {
    providers.value = snapshot.providers;
    models.value = snapshot.models;
  }

  async function loadProviderModelSnapshot() {
    return loadClientResource<api.ProviderModelSnapshot>({
      key: "provider-model",
      ...
      read: async () => {
        ...
        const models = Object.fromEntries(
          await Promise.all(
            providers.map(
              async (provider) => [provider.id, await api.listProviderModels(provider.id)] as const,
            ),
          ),
        );
        return { providers, models };
      },
      apply: applyProviderModelSnapshot,
      loading: (value) => {
        root.loading.providers = value;
      },
    });
  }
  ...
  async function fetchProviders() {
    root.clearError();
    try {
      await loadProviderModelSnapshot();
  ...
  async function fetchModels(providerId: string) {
    ...
    try {
      await loadProviderModelSnapshot();
      return models.value[providerId] ?? [];
    } catch (err) {
...
export const useResourceStore = defineStore("resource", () => {
  ...
  const allResources = computed(() => {
    ...
    return [
      ...
      ...globalResources.value.plugins.map((e) => ({ kind: "plugins" as const, info: e })),
  ...
  const getResourcesByKind = computed(() => (kind: ResourceKind) => {
    ...
    switch (kind) {
      ...
      case "plugins":
        return globalResources.value.plugins;
      case "mcp":
