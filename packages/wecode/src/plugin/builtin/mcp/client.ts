...
import type { McpServerConfigType, McpServerStatus } from "./types.js";
import { mcpToolToPluginTool, type McpToolDefinition } from "./tool.js";
...
export class McpClientManager {
  ...
  /** 已转换为插件工具的 MCP 工具缓存。 */
  private toolsCache: Array<ToolDefinition<TSchema, unknown>> | null = null;
  /** 正在进行的连接任务，用于合并并发 connectAll 调用。 */
  private connectPromise: Promise<void> | null = null;
  /** 内置插件从数据库绑定中传入的 MCP 服务配置。 */
  ...
  /** 返回全部已连接 MCP 服务转换出的插件工具。 */
  getTools(): Array<ToolDefinition<TSchema, unknown>> {
  ...
  /** 从已连接服务的工具定义构建插件工具列表。 */
  private buildTools(): Array<ToolDefinition<TSchema, unknown>> {
    const tools: Array<ToolDefinition<TSchema, unknown>> = [];
    ...
    for (const [, state] of this.servers) {
      ...
      for (const mcpTool of state.tools) {
        tools.push(
          mcpToolToPluginTool(state.serverName, mcpTool, async (toolName, params) => {
