...
export class CodexSessionRuntime extends ExternalSessionRuntime {
      ...
      try {
        await timedSessionStep(sessionId, "codex/initialize", () =>
          runtime.request("initialize", {
            clientInfo: { name: "wecode", title: "WeCode", version: "0.1.0" },
  ...
  protected async runExternalTurn(message: string, images?: ImageContent[]): Promise<void> {
    ...
    try {
      ...
      // Long coding turns may legitimately run for several minutes; do not fail them at 3m.
      const turnWaitMs = 1_800_000;
...
/**
 * IDE/service-launched Wecode often misses proxy variables that exist in
