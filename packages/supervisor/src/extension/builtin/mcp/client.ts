...
import { mcpToolToExtensionTool, type McpToolDefinition } from "./tool.js";
  ...
  private buildTools(): Array<ToolDefinition<TSchema, unknown>> {
    ...
    for (const [, state] of this.servers) {
      ...
      for (const mcpTool of state.tools) {
        tools.push(
          mcpToolToExtensionTool(state.serverName, mcpTool, async (toolName, params) => {
