...
import { listWecodeCommitsBetween, type WecodeCommitInfo } from "../../utils/git.js";
...
function dailyWorkDir(): string {
  const dir = join(getWecodeHome(), "daily-work");
...
export async function runDailyWorkAnalysis(
  ...
  for (const project of db.listProjects()) {
    const commits = await listWecodeCommitsBetween(project.cwd, startMs, endMs);
  ...
  if (sections.length) {
    ...
    summary = `${dayKey} 没有发现 wecode (sv) commit。`;
  }
...
export async function maybeRunDailyWorkCatchUp(db: WecodeDb): Promise<void> {
...
export function startDailyWorkScheduler(db: WecodeDb): () => void {
