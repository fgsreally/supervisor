 ...
 */
function parsePythonSymbols(
  filePath: string,
): Array<{ name: string; kind: string; line: number; character: number }> {
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { action: "definition", path: relPath, count: 0, locations: [] },
            null,
            2,
          ),
        },
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { action: "references", path: relPath, count: 0, locations: [] },
            null,
            2,
          ),
        },
    ...
    if (stderr) {
      ...
      for (const line of stderr.split("\n")) {
        ...
        const errMatch = line.match(
          /^(SyntaxError|IndentationError|NameError|TypeError|ValueError|ImportError|ModuleNotFoundError|AttributeError|TabError):\s*(.+)/,
        );
        if (errMatch) {
          diagnostics.push({
            path: relPath.replace(/\\/g, "/"),
      ...
      try {
        ...
        for (const line of innerStderr.split("\n")) {
          ...
          if (match) {
            ...
            const msgEnd = line.includes("SyntaxError")
              ? (line.split("SyntaxError")[1]?.trim() ?? line)
              : line;
            diagnostics.push({
              path: relPath.replace(/\\/g, "/"),
              line: errLine,
  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        ...
        symbol: {
          type: "string",
          description: "Optional symbol name fallback for position lookup.",
        },
    ...
    async execute(_toolCallId, rawParams) {
      ...
      if (!validActions.includes(params.action)) {
        throw new Error(
          "lsp action must be one of symbols|definition|references|rename|diagnostics",
        );
      }
      ...
      if (!isSupportedFile(absolutePath)) {
        throw new Error(
          `lsp currently supports TS/JS, Python, and Go files only (got: ${extname(absolutePath)})`,
        );
      }
      ...
      if (params.action === "diagnostics") {
        ...
        const items = [...syntactic, ...semantic].map((diag) =>
          formatDiagnostic(absolutePath, diag, cwd),
        );
        return {
          content: [
            {
      ...
      if (params.action === "symbols") {
        ...
        const filtered =
          typeof params.symbol === "string" && params.symbol.trim().length > 0
            ? rows.filter((row) =>
                row.name.toLowerCase().includes(params.symbol!.trim().toLowerCase()),
              )
            : rows;
        return {
          content: [
      ...
      if (params.action === "rename") {
        ...
        if (!renameInfo.canRename) {
          throw new Error(
            renameInfo.localizedErrorMessage ?? "Rename not available at this position",
          );
        }
        const locations =
      ...
      if (params.action === "definition") {
        ...
        const results = defs.map((item) => {
          ...
          const lc = file
            ? file.getLineAndCharacterOfPosition(item.textSpan.start)
            : { line: 0, character: 0 };
