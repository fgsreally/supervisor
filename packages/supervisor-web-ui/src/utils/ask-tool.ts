...
export function parseAskResultDetails(resultContent?: Array<{ type: string; text: string }>): AskResultDetails | null {
...
export function normalizeAskToolResult(result: unknown): {
	...
	const details =
		record.details && typeof record.details === "object" ? (record.details as AskResultDetails) : undefined;
	...
	const contentFromResult = Array.isArray(record.content)
		...
		: undefined;
	const text = contentFromResult?.find((p) => p.type === "text")?.text?.trim() || summary || "已回答";
	return {
