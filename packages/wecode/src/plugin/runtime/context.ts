...
import { buildPluginDeps, createPluginDatabase } from "./deps.js";
...
type ContextDbOptions = PluginSqliteDatabase | undefined;
...
export type Database = WecodeDb;

interface ContextPluginHost {
  ...
  unregisterTool(pluginId: string, name: string): void;
  registerCommand(pluginId: string, name: string, definition: PluginCommandDefinition): void;
  unregisterCommand(pluginId: string, name: string): void;
  callTool(name: string, params: unknown, signal?: AbortSignal): Promise<PluginToolCallResult>;
  removeResources(pluginId: string): void;
}

/** Session-scoped context shared by every plugin activated for that session. */
export class Context {
  ...
  readonly jobs: import("../types.js").PluginJobFacade;
  readonly db: ContextDb;
  readonly project: PluginContext["project"];
  readonly ui: {
  ...
  /** Internal session resource bridge used by built-in plugins. */
  readonly agentResource: AgentResource;
  readonly watson: PluginContext["watson"];

  private activePluginId: string | undefined;
  private readonly pluginCleanups = new Map<string, Set<() => void>>();
  private pluginHost: ContextPluginHost | undefined;
  ...
  constructor({ sessionManager, db, sessionRuntime, resource }: ContextDependencies) {
    ...
    const pluginDb = createPluginDatabase({
    ...
    const deps = buildPluginDeps({
      ...
      emitPluginEvent: (event) => this.pluginHost?.emit(event),
    });

    this.services = new SessionPluginServices({
      sessionId: session.id,
      deps: {
    ...
    const sessionTools: ContextSessionTools = {
      ...
      beforeUse: (handler, options) =>
        this.trackPluginCleanup(this.services.tools.beforeUse(handler, options)),
      afterUse: (handler, options) =>
        this.trackPluginCleanup(this.services.tools.afterUse(handler, options)),
      activate: async (names) => {
        this.requirePluginHost().setToolsActive(names, true);
        await deps.syncActiveTools();
      },
      deactivate: async (names) => {
        this.requirePluginHost().setToolsActive(names, false);
        await deps.syncActiveTools();
      },
    ...
    const projectFacade: PluginContext["project"] = {
      data: {
        get: async () => {
    ...
    const injectFacade: PluginContext["inject"] = {
    ...
    const sessionOptions: ContextSessionOptions = {
      ...
      messages: {
        list: pluginDb.getMessages,
        get: async (messageId) => {
          const message = await pluginDb.getMessageById(messageId);
          if (!message) return undefined;
          const meta: MessageMetaFacade = {
            get: async () => pluginDb.getMessageMeta(messageId),
            set: async (nextMeta) => deps.setMessageMeta(messageId, nextMeta),
            patch: (patch) => deps.patchMessageMeta(messageId, patch),
          };
          return { ...message, meta };
        },
        tree: pluginDb.getMessageTree,
        currentBranch: pluginDb.getCurrentBranch,
        search: pluginDb.searchMessages,
        getMeta: pluginDb.getMessageMeta,
        setMeta: deps.setMessageMeta,
        patchMeta: deps.patchMessageMeta,
        setLabel: deps.setLabel,
        stats: pluginDb.getMessageStats,
        contextUsage: pluginDb.getContextUsage,
      },
      data: {
      ...
      getParent: pluginDb.getParentSession,
      children: pluginDb.getChildSessions,
    ...
    const agentOptions: ContextAgentOptions = {
      ...
      registerTool: (definition) =>
        this.requirePluginHost().registerTool(this.requireActivePlugin(), definition),
      unregisterTool: (name) =>
        this.requirePluginHost().unregisterTool(this.requireActivePlugin(), name),
      activate: async (names) => {
        this.requirePluginHost().setToolsActive(names, true);
        await deps.syncActiveTools();
      },
      ...
      registerCommand: (name, definition) =>
        this.requirePluginHost().registerCommand(
          this.requireActivePlugin(),
          name,
          definition,
        ),
      unregisterCommand: (name) =>
        this.requirePluginHost().unregisterCommand(this.requireActivePlugin(), name),
    ...
    this.tools = {
      list: () => this.requirePluginHost().listTools(),
      get: (name) =>
        this.requirePluginHost()
          .listTools()
      ...
      call: async <TResult = unknown>(
        ...
        (await this.requirePluginHost().callTool(
          name,
          params,
          options?.signal,
        )) as PluginToolCallResult<TResult>,
    };
    this.jobs = {
    ...
    this.watson = {
      run: ((options: {
        ...
        })) as PluginContext["watson"]["run"],
    };
  }

  /** Internal bridge used by the session's plugin runtime. */
  attachPluginHost(host: ContextPluginHost): void {
    if (this.pluginHost && this.pluginHost !== host) {
      throw new Error(`Context for session ${this.session.id} is already attached`);
    }
    this.pluginHost = host;
  }

  /** Run one plugin with a temporary ownership marker on this session context. */
  async runPlugin<T>(pluginId: string, run: () => T | Promise<T>): Promise<T> {
  ...
  runPluginSync<T>(pluginId: string, run: () => T): T {
    const previous = this.activePluginId;
    this.activePluginId = pluginId;
    try {
      return run();
  ...
  removePluginResources(pluginId: string): void {
    ...
    this.requirePluginHost().removeResources(pluginId);
  }
  ...
  on<K extends PluginEvent["type"]>(
    ...
    return this.requirePluginHost().on(this.requireActivePlugin(), event, handler, options);
  }
  ...
  private listSessionTools(builtinTools: AgentTool[]): ToolInfo[] {
    ...
    for (const tool of this.pluginHost?.listTools() ?? []) merged.set(tool.name, tool);
    return [...merged.values()];
  }

  private trackPluginCleanup(cleanup: () => void): () => void {
    const pluginId = this.requireActivePlugin();
    const cleanups = this.pluginCleanups.get(pluginId) ?? new Set<() => void>();
    let active = true;
    const tracked = () => {
      if (!active) return;
      active = false;
  ...
  private requireActivePlugin(): string {
    if (!this.activePluginId) {
      throw new Error("Plugin resources can only be registered during plugin activation");
    }
    return this.activePluginId;
  }

  private requirePluginHost(): ContextPluginHost {
    if (!this.pluginHost) throw new Error("Context is not attached to a plugin runtime");
...
export class ContextAgent {
  ...
  registerSlash(name: string, definition: PluginCommandDefinition): void {
...
/** Raw SQL access for advanced plugin use cases. */
export class ContextDb {
  ...
  private getDatabase(): PluginSqliteDatabase {
