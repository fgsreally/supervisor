...
import { storePendingEditApproval } from "./edit-approval.js";
import { createGitSnapshot, findGitRoot } from "../../utils/git.js";
  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        path: {
          type: "string",
          description: "File path (relative to current session cwd or absolute).",
        },
        requireApproval: {
          type: "boolean",
          ...
          items: {
            ...
            properties: {
              ...
              beforeText: {
                type: "string",
                description:
                  "Optional anchor text expected before oldText (within anchorWindowChars).",
              },
              afterText: {
                type: "string",
                description:
                  "Optional anchor text expected after oldText (within anchorWindowChars).",
              },
              anchorWindowChars: {
                type: "number",
        ...
        anchorWindowChars: {
          type: "number",
          description: "Legacy single-edit anchor scan window size.",
        },
      },
      required: ["path"],
    },
          ...
          const approval: PendingEditApproval = {
            ...
            resolve: async (approved: boolean) => {
              ...
              if (approved) {
                try {
                  ...
                  if (options?.logFile) {
                    void options.logFile
                      .log("info", `edit applied to ${params.path}`, ["edit"], {
                        filePath: params.path,
                        edits: edits.length,
                        diff,
                      })
                  ...
                  resolve({
                    content: [
                      {
                        type: "text",
                        text: `Updated ${params.path} with ${edits.length} edit(s).`,
                      },
                    ],
                ...
                resolve({
                  content: [
                    { type: "text", text: `Edit rejected for ${params.path}. No changes made.` },
                  ],
