...
import type { UIFileItem, UIResourceItem, UIResourceKind, UISkillItem, UISkillFile } from "@/types/ui";
...
function skillFilesToUi(skillPath: string, files: Array<{ relativePath: string; content: string }>): UISkillFile[] {
...
function extensionFilesToUi(extPath: string, files: Array<{ relativePath: string; content: string }>): UISkillFile[] {
	return files.map((file) => ({
...
export function agentResourcesToUiItems(agentId: string, resources: AgentResources): UIResourceItem[] {
	return layerToUiItems(agentId, resources.layers.agent, "agent");
}

/** Agent tab shows only resources under the agent home directory. */
...
export function getResourcesByKind(items: UIResourceItem[], kind: UIResourceKind): UIResourceItem[] {
