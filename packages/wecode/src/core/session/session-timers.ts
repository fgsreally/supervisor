...
/** Timer definition stored in sessions.meta.timers (plugin config, not a Job). */
...
export function listSessionTimers(db: WecodeDb, sessionId: number): SessionTimer[] {
...
export function deleteSessionTimer(db: WecodeDb, sessionId: number, timerId: string): boolean {
  const current = listSessionTimers(db, sessionId);
