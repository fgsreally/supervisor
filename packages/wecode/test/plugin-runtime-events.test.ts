...
import { createEventBus, SessionPluginHost } from "../src/plugin/runtime/index.js";
...
describe("plugin runtime events", () => {
  ...
  it("attaches outputs to the latest assistant message at agent end", async () => {
    const sessionDir = join(tmpdir(), `wecode-message-assets-test-${Date.now()}`);
  ...
  it("exposes project-owned and session-owned directories to plugins", async () => {
    const runtime = new SessionPluginHost(createPluginTestContext(createRuntimeOptions()));
    let seenDir = "";
    let seenProjectDir = "";
  ...
  it("registers subagent tool only on main sessions", async () => {
    const mainRuntime = new SessionPluginHost(
      createPluginTestContext(createRuntimeOptions()),
    );
    const childRuntime = new SessionPluginHost(
  ...
  it("runs eval code in a persistent session-owned JavaScript kernel", async () => {
    const sessionDir = join(tmpdir(), `wecode-eval-${Date.now()}`);
    const options = createRuntimeOptions();
    options.sessionDir = sessionDir;
    const runtime = new SessionPluginHost(createPluginTestContext(options));
    await runtime.initialize();
  ...
  it("terminates a timed-out JavaScript worker and starts a fresh one", async () => {
    const options = createRuntimeOptions();
    options.sessionDir = join(tmpdir(), `wecode-eval-timeout-${Date.now()}`);
    const runtime = new SessionPluginHost(createPluginTestContext(options));
    await runtime.initialize();
  ...
  it("persists todo, goal, plan, and current task in session metadata", async () => {
    const sessionDir = join(tmpdir(), `wecode-tasks-${Date.now()}`);
    const options = createRuntimeOptions();
    options.sessionDir = sessionDir;
  ...
  it("loads plugin files that listen for session.end", async () => {
    const tmp = join(tmpdir(), `ext-events-${Date.now()}`);
    mkdirSync(join(tmp, "plugins"), { recursive: true });
    const helloDir = join(tmp, "plugins", "hello");
    mkdirSync(helloDir, { recursive: true });
    const pluginFile = join(helloDir, "index.ts");
    writeFileSync(
      pluginFile,
...
export default definePlugin({
  ...
  setup(ctx) {
    ...
    const result = await loadPlugins([pluginFile]);
    expect(result.errors).toHaveLength(0);
    expect(result.plugins.some((ext) => ext.definition.name === "hello-events")).toBe(true);
