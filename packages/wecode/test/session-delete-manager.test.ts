...
describe("session deletion", () => {
  it("closes the runtime before removing its worktree", async () => {
    root = join(tmpdir(), `wecode-delete-test-${Date.now()}`);
    const projectCwd = join(root, "project");
    const worktreePath = join(projectCwd, ".wecode", "worktrees", "1");
    ...
    const runtimes = (
      manager as unknown as {
        runtimes: Map<number, { plugin: null; clear: () => Promise<void> }>;
      }
    ).runtimes;
    runtimes.set(session.id, { plugin: null, clear });
  ...
  it("clears a runtime deleted by another Wecode process", async () => {
    root = join(tmpdir(), `wecode-delete-test-${Date.now()}`);
    ...
    internal.runtimes.set(session.id, { plugin: null, clear });
