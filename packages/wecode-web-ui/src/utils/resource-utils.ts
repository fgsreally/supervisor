...
export function isFileItem(item: UIResourceItem): item is UIFileItem {
  return item.kind === "plugins" || item.kind === "prompts" || item.kind === "mcp";
}
...
export function relativeWecodePath(path: string): string {
  const normalized = path.trim().replaceAll("\\", "/");
  const withoutHome = normalized.replace(/^~\/\.pi\/wecode\//i, "");
  if (withoutHome !== normalized) return withoutHome.replace(/^global\//i, "");

  const marker = "/.pi/wecode/";
