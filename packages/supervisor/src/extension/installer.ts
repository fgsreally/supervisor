...
export function installExtensionToGlobal(source: string): InstallResult {
  ...
  const globalExtDir = ensureGlobalResourceDirectory("extensions");
...
export function uninstallGlobalExtension(id: string): void {
  const globalExtDir = getGlobalResourceDirectory("extensions");
 ...
 */
function getGlobalExtensionDir(id: string): string | null {
  const globalExtDir = getGlobalResourceDirectory("extensions");
  const target = join(globalExtDir, id);
