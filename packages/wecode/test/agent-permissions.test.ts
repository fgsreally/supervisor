...
function makeRoot(name: string): string {
  const root = join(tmpdir(), `wecode-permission-${name}-${Date.now()}-${Math.random()}`);
...
describe("agent permission rules", () => {
  ...
  it("treats project-root paths as project/** when cwd is a session worktree", () => {
    ...
    const worktree = join(project, ".wecode", "worktrees", "42");
  ...
  it("enforces permissions without a plugin or Project", async () => {
  ...
  it("setTools only exposes active tools to the model", async () => {
    ...
    // Simulate plugin registry: Setup inactive, Start active
    (
