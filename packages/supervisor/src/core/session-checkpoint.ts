...
import { createGitSnapshot, restoreGitSnapshot } from "../git/git-worktree.js";
...
export function parseCheckpoints(meta: Record<string, unknown>): SessionCheckpoint[] {
	...
	for (const item of raw) {
		...
		if (typeof row.id !== "string" || typeof row.entryId !== "string" || typeof row.createdAt !== "number") {
			continue;
		}
		checkpoints.push({
			id: row.id,
...
export async function createSessionCheckpoint(
	...
	const existing = parseCheckpoints(typeof session.meta === "string" ? JSON.parse(session.meta) : session.meta);
...
export function listSessionCheckpoints(db: Pick<SupervisorDb, "get">, sessionId: number): SessionCheckpoint[] {
	const session = db.get(sessionId);
	if (!session) throw new Error(`Session ${sessionId} not found`);
	return parseCheckpoints(typeof session.meta === "string" ? JSON.parse(session.meta) : session.meta);
}
...
export async function rewindSessionToCheckpoint(
	...
	const checkpoints = parseCheckpoints(typeof session.meta === "string" ? JSON.parse(session.meta) : session.meta);
