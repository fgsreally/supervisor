...
function buildWecodeBash(cwd: string, options?: DefaultToolsOptions): AgentTool {
  const merged = resolveToolsOptions(cwd, options);
  const bashOptions: WecodeBashOptions = {
    cwd,
  ...
  return createWecodeBashTool(bashOptions);
}

/**
 * Default wecode tools: pi coding tools with Wecode bash (fg + bg) replacing pi bash.
...
export function createDefaultTools(
  ...
  const merged = resolveToolsOptions(cwd, options);
  const bash = buildWecodeBash(cwd, options);
  const sessionDir =
