...
import { buildExtensionDeps, createExtensionDatabase } from "../extension-system/extension-deps.js";
import {
...
import type { ToolPolicy } from "../extension-system/tool-policy.js";
import type {
  BroadcastEvent,
  EventBus,
  EventHandlerContext,
...
import { getProjectDir, getSessionDir } from "./session-files.js";
...
export interface ContextDependencies {
  ...
  db: Database;
  sessionRuntime: SupervisorSessionRuntime;
}
...
export class ContextSession {
  ...
  sendParentMsg(content: string, options?: { level?: number }): Promise<void> {
    return this.options.sendParentMsg(content, options);
  }
  pausing<T>(reason: string, work: Promise<T> | (() => Promise<T>)): Promise<T> {
