...
import type { PluginContext, PluginDefinition, PluginJobFacade } from "../../types.js";
import {
...
const UPDATE_PARAMS = Type.Object({
  ...
  startCommand: Type.Optional(
    Type.String({
      description:
        "Start command; required for add/update. Use a start script declared by the project and listen on 127.0.0.1 or 0.0.0.0; do not invoke a bare binary resolved from Wecode PATH. Declare ports with consecutive placeholders such as ${PORT1} and ${PORT2}; the tool allocates and replaces them after the call.",
    }),
  ),
  port: Type.Optional(
    Type.Number({
...
function pluginJobsAsHost(jobs: PluginJobFacade): SessionServiceJobHost {
  return {
...
function ensureGlobalSleepScheduler(ctx: PluginContext): void {
...
const projectServicesPlugin: PluginDefinition = {
  name: "service",
  async setup(ctx) {
    const jobs = pluginJobsAsHost(ctx.jobs);
