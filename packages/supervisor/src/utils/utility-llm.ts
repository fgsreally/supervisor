...
export function resolveUtilityModelConfig(settings: SupervisorSettings = readSupervisorSettings()): {
	provider: string;
	modelId: string;
} | null {
...
export async function compactWithUtilityModel(
	...
	return compact(preparation, auth.model, auth.apiKey, auth.headers, customInstructions, undefined, "off");
}
