...
async function onCompleteSession() {
  ...
  const confirmed = window.confirm('完成会话将把 worktree 分支合并到主分支，并关闭此会话。请先提交所有变更。继续？')
  if (!confirmed) return
  stopStreaming()
  try {
...
async function onRewindSession() {
  ...
  try {
    ...
    const confirmed = window.confirm(`回滚到存档点 ${target.label ?? target.id.slice(0, 8)}？将恢复代码与会话位置。`)
    if (!confirmed) return
    stopStreaming()
...
function attachToRunningSession() {
  ...
  const lastAssistant = [...groups]
    .reverse()
    .find((g): g is Extract<DisplayGroup, { type: 'grouped_assistant' }> => g.type === 'grouped_assistant')
