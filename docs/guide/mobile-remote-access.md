...
通过 Cloudflare Quick Tunnel，在手机上扫码访问本机 Wecode（HTTPS）。无需 Cloudflare 账号或域名；`cloudflared` 由 npm 依赖自动下载。
...
`--cwd <path>` = Wecode **全局根**。指定后以下全部落在该目录下：

| 内容        | 路径                                       |
| ----------- | ------------------------------------------ |
| 数据库      | `<cwd>/wecode.db`                      |
| public      | `<cwd>/public/`                            |
| global 资源 | `<cwd>/global/{skills,plugins,prompts}` |
| agents      | `<cwd>/agents/{id}/`                       |
| projects    | `<cwd>/projects/{id}/`                     |
| media       | `<cwd>/media/`                             |
| settings    | `<cwd>/settings.json`                      |

不传 `--cwd` 时默认全局根为 `~/.wecode`。
...
Android / iOS 壳见 `packages/wecode-mobile`。Android 16 Live Updates 见 [Android Live Updates 接入指南](./android-live-updates.md)。

## 安全

- 二维码不含 PIN；公网 URL 仍靠 6 位 PIN 门禁
- 适合个人临时远程；关闭 wecode 即断隧道
