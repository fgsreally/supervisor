...
import type { PluginSession } from "../../plugin/types.js";
...
export function applySessionActivityPolicy(session: PluginSession): void {
...
export function touchSessionActivity(db: WecodeDb, id: number, at = Date.now()): void {
  const row = db.get(id);
  if (!row) return;
