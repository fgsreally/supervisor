...
export function minimizeOutput(command: string, stdout: string): MinimizerResult {
	...
	if (!identity) {
		return { minimized: false, text: stdout, originalLines: stdout.split("\n").length, minimizedLines: stdout.split("\n").length };
	}
	...
	for (const filter of BUILTIN_FILTERS) {
		...
		if (filter.maxLines !== undefined && processed.length > filter.maxLines) {
			...
			processed.push(`[... ${omitted + (originalLines - filter.maxLines > 0 ? originalLines - filter.maxLines - omitted : 0)} lines omitted]`);
		}
...
const isMain = process.argv[1]?.endsWith("output-minimizer.ts") || process.argv[1]?.endsWith("output-minimizer.js");
if (isMain) {
	demoMinimizer();
}
