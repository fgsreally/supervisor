...
/** Extra dirs often missing when wecode is launched from IDE / service managers. */
function extraUserBinDirs(): string[] {
  ...
  if (platform() === "win32") {
    return [
      ...
      // even when Wecode was launched from an IDE with a trimmed environment.
...
export function resolveExecutable(command: string, env = process.env): string | null {
  ...
  for (const directory of pathSearchDirs(env)) {
    for (const plugin of plugins) {
      const candidate = join(directory, `${command}${plugin}`);
...
/** Windows `.cmd`/`.bat` need `shell: true`; compare plugin case-insensitively. */
