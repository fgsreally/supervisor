 ...
 * Reuses the same URL shapes as plugin install (github web, git+, .git remotes).
 */
...
import { parseGithubUrl, type ParsedGitSource } from "../plugin/installer.js";
...
export function cloneGitSourceToTemp(source: ParsedGitSource): {
  ...
  const tempRoot = mkdtempSync(join(tmpdir(), "wecode-resource-git-"));
