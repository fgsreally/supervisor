...
const originalLocale = process.env.WECODE_LOCALE;

afterEach(() => {
  if (originalLocale === undefined) delete process.env.WECODE_LOCALE;
  else process.env.WECODE_LOCALE = originalLocale;
});

describe("console log i18n", () => {
  it("selects Chinese from the explicit locale", () => {
    process.env.WECODE_LOCALE = "zh-CN";
    expect(getLogLocale()).toBe("zh-CN");
    expect(translateLog("cli.providers.added", { id: "demo" })).toBe("已添加提供商：demo");
  });

  it("falls back to English for unsupported locales", () => {
    process.env.WECODE_LOCALE = "fr-FR";
