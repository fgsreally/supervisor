...
function pluginSlug(source: string, index: number): string {
  const raw = basename(resolve(source))
    .toLowerCase()
    .replace(/[^a-z0-9_-]+/g, "-");
  return raw || `plugin-${index + 1}`;
}

async function readMessages(db: WecodeDb, sessionId: number): Promise<AgentMessage[]> {
...
export async function createAiTestEnvironment(
  ...
  const root = mkdtempSync(join(tmpdir(), "wecode-ai-test-"));
  ...
  const db = new WecodeDb(join(root, "wecode.db"));
  ...
  for (const [index, source] of (options.plugins ?? []).entries()) {
    const slug = pluginSlug(source, index);
    ...
    await manager.getPluginRegistry().reload(db, slug);
  }
