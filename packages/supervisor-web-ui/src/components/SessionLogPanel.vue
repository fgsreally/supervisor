<template>
  <div class="session-log-panel flex flex-col h-full w-full overflow-hidden" style="background: var(--app-settings-bg)">
    <!-- Header -->
    <div class="h-14 md:h-16 border-b flex items-center px-4 shrink-0 gap-3" style="background: var(--app-settings-bg); border-color: var(--app-border)">
      <button type="button" class="p-1.5 rounded-md" style="color: var(--app-text-secondary)" @click="$emit('close')">
    ...
    <!-- Filter bar -->
    <div class="px-4 py-3 border-b space-y-2 shrink-0" style="border-color: var(--app-border-subtle)">
      <div class="flex flex-wrap gap-2">
        <button
          ...
          :style="levelFilter === lv.key ? `background: ${lv.color}22; border-color: ${lv.color}; color: ${lv.color}` : 'border-color: var(--app-border); color: var(--app-text-muted)'"
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar">
      <div v-if="!filteredEntries.length" class="py-12 text-center text-[13px]" style="color: var(--app-text-muted)">暂无日志</div>
      <div
      ...
      >
        <div class="flex items-center gap-2 mb-1">
          <span class="text-[11px] shrink-0 font-mono" style="color: var(--app-text-muted)">{{ formatTime(entry.t) }}</span>
          <span
          ...
          <span v-for="tag in entry.tags" :key="tag" class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium" :style="{ background: tagBg(tag), color: tagColor(tag) }">
            {{ tag }}
          </span>
        </div>
        <div class="font-mono text-[12px] whitespace-pre-wrap break-all" style="color: var(--app-text-primary)">{{ entry.m }}</div>
      </div>
    </div>
  </div>
</template>
...
const COLOR_WHEEL = ['#6366f1', '#f59e0b', '#10b981', '#3b82f6', '#ec4899', '#14b8a6', '#8b5cf6', '#f97316']
