...
public class WecodeNativePlugin extends Plugin {

    private static WecodeNativePlugin instance;
    ...
    public void startBackgroundConnection(PluginCall call) {
        String title = call.getString("title", "Wecode");
        String body = call.getString("body", "连接中");
        Intent intent = new Intent(getContext(), WecodeForegroundService.class);
        intent.setAction(WecodeForegroundService.ACTION_START);
        intent.putExtra(WecodeForegroundService.EXTRA_TITLE, title);
        intent.putExtra(WecodeForegroundService.EXTRA_BODY, body);
    ...
    public void stopBackgroundConnection(PluginCall call) {
        Intent intent = new Intent(getContext(), WecodeForegroundService.class);
        intent.setAction(WecodeForegroundService.ACTION_STOP);
        getContext().startService(intent);
        call.resolve();
    }
    ...
    public void updateBackgroundConnection(PluginCall call) {
        String title = call.getString("title", "Wecode");
        String body = call.getString("body", "连接中");
        WecodeForegroundService.updateNotification(getContext(), title, body);
        call.resolve();
    }
    ...
    private void launchQrScanner(PluginCall call) {
        ...
        options.setPrompt("扫描电脑端 Wecode 二维码");
