...
/**
 * 为 Plugin 创建事件总线。
 * 使用 Node.js EventEmitter 实现同一 Agent 内的插件间通信。
 */
...
/**
 * 构建 Plugin 内部运行实现需要的依赖。
 * 将所有能力连接到 SessionRuntime、SessionManager 和 WecodeDb。
 */
export function buildPluginDeps(deps: {
  ...
  const { runtime, manager, db, sessionId, projectId, listSessionTools, emitPluginEvent } = deps;
  ...
  return {
    ...
    sendMessage: async (message) => {
      ...
      if (message.triggerTurn) {
        void runtime
          .prompt(`[Plugin:${message.customType}]\n${message.content}`)
          .catch((err: unknown) => {
            const msg = err instanceof Error ? err.message : String(err);
            sessionLogEvent(sessionId, "error", "runtime.pluginMessageFailed", { error: msg });
          });
      }
    },
    ...
    sendToChild: async (
      ...
      await manager.submitSubagentInput(sessionId, targetSessionId, content, {
        source: options?.source ?? `plugin:parent:${sessionId}`,
    ...
    getMemberAgentsByTag: async (tag: string): Promise<MemberAgentInfo[]> => {
      // Session membership no longer stores tags. Keep the plugin API stable.
      void tag;
      return [];
    },
    ...
    pausing: async <T>(reason: string, work: Promise<T> | (() => Promise<T>)): Promise<T> => {
      ...
      try {
        ...
        if (reason.trim()) {
          ...
          sessionLogEvent(sessionId, "debug", "runtime.pluginPauseResolved", {
            id: sessionId,
            reason,
          });
        }
    ...
    getSignal: () => {
      // Plugin 自己的 AbortSignal，不是 Harness 内部信号。
      return undefined;
    },
    ...
    switchSession: async (_targetSessionId: number) => {
      // Session-scoped: switching the plugin context is a no-op for now
      // 每个 Agent 会话都有自己的 Plugin，因此这里不切换实例。
      // 跨会话通信应使用事件总线。
    },
    ...
    navigateTree: async (
      ...
      // TODO: implement navigateTree for plugins when the use case arises
    },
    ...
    compact: async (options?: { customInstructions?: string }) => {
      ...
      await emitPluginEvent(beforeEvent);
      if (beforeEvent.cancel) {
        throw new Error("Compaction cancelled by plugin");
      }
...
interface DbAdapterOptions {
  ...
  sqlite?: PluginDatabase["sqlite"];
}

/**
 * 创建数据库访问对象
 */
export function createPluginDatabase(options: DbAdapterOptions): PluginDatabase {
